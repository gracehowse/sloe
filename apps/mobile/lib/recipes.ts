...
export function useDiscoverRecipes() {
  ...
  const refresh = useCallback(async () => {
    ...
    if (!error && data) {
      ...
      // tester stage. Seeds are content-curated, not algorithmic;
      // they take stable precedence ahead of any DB-sourced rows.
      // Seed entries carry `feedSource: "catalog"` so downstream UI
      // can distinguish them from community uploads when needed.
