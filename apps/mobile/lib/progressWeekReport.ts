...
function sumDay(meals: JournalMeal[]) {
  return meals.reduce(
    (acc, m) => ({
...
export function getStreakContributingDays(byDay: ByDay, now: Date = new Date()): Array<{ key: string; mealCount: number; calories: number }> {
