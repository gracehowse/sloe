...
**`PhaseScriptExecution` failed** on **hermes-engine** or **ReactNativeDependencies:** Xcode’s script phases often don’t see `node` on `PATH` (Homebrew isn’t on `PATH` there). **`npx expo prebuild --platform ios`** rewrites **`ios/.xcode.env`** (adds `/opt/homebrew/bin` + `/usr/local/bin` to `PATH` and Node fallbacks) and sets **`NODE_BINARY`** in **`ios/.xcode.env.local`**. You can also edit those files manually; **`which node`** in Terminal gives the path to paste. In Xcode: **Report navigator** (⌘9) → select the failed build → expand the red **Run script** row to read the real stderr (e.g. `node: command not found`). If you enabled **User Script Sandboxing** on the target, turn it **off** for **Debug** (it blocks some RN/CocoaPods scripts).
