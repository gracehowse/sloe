...
export function HouseholdCard() {
    ...
    return (
      <View style={{ backgroundColor: t.elevated, borderRadius: Radius.lg, borderWidth: 1, borderColor: t.border, padding: 16, marginBottom: 14 }}>
        ...
        <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 8 }}>
          <View style={{ width: 28, height: 28, borderRadius: Radius.full, backgroundColor: accent.primarySoft, alignItems: "center", justifyContent: "center" }}>
  ...
  return (
    <View style={{ backgroundColor: t.elevated, borderRadius: Radius.lg, borderWidth: 1, borderColor: t.border, padding: 16, marginBottom: 14 }}>
      ...
      {/* Members list. Per F-16 legal approval, other members show name +
          role only by default — private unless opted in via `share_targets`
          (H4). `getMyHousehold()` sources an opted-in co-member's numbers via
          the `get_household_shared_targets` RPC (ENG-1602, re-verifies
          co-membership + opt-in server-side); this UI just checks `m.remaining`.
