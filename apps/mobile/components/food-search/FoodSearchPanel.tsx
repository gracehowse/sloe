...
import { optionalSanitizedMicrosPer100g } from "@suppr/nutrition-core/microPlausibility";
import {
  computeFoodSearchHistoryAndFavorites,
  resolveFavoriteToggleId,
} from "@suppr/nutrition-core/foodSearchHistoryAndFavorites";
import { mergeFetchedMicrosPer100g } from "@suppr/nutrition-core/foodSearchMicrosMerge";
...
/**
 * Redesign (ENG-814, `redesign_search_results` collapsed permanently-on
 * ENG-1651) — discriminated FlatList feed row for the sectioned,
 ...
 * corners + inset seams render correctly.
 */
...
export default function FoodSearchPanel({
  ...
  // `redesign_search_results` collapsed permanently-on ENG-1651): unified
  // segmented control + softly-elevated grouped result cards + "Best
  // matches"/"More results" section headers + a legible confidence chip
  // (soft-blue Verified / amber Estimated) driven by each row's
  // `confidenceTier`. Matches
  ...
  // shadow + no border; dark = tonal lift + hairline).
  ...
  const handleCreateCustomFood = useCallback(
    async (payload: CreateCustomFoodPayload) => {
      ...
      // to the sheet's handleSave, which reveals the "save anyway" override.
  ...
  // broad catalogue (history wins). Group renders on the All/Recents
  // filters (Branded/Generic/Custom intents are explicitly "search-result
  // shape" and shouldn't be topped with history/favourites).
  //
  ...
  // and not a cross-platform hook. Mobile's "Recents" category (which web
  // does not have, ENG-748 #8) is preserved exactly via this platform's own
  // gate expression, not collapsed into web's.
  const isHistoryVisibleCategory = activeCategory === "All" || activeCategory === "Recents";
  const {
    favoriteKeys,
    favoriteMatches,
    ...
    <View style={{ flex: 1 }}>
      ...
      >
        {CATEGORY_LIST.map((cat) => {
          ...
          // Redesign (ENG-814, `redesign_search_results` collapsed
          // permanently-on ENG-1651): one unified segmented control — softly
