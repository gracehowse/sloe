...
export default function DiscoverScreen() {
  ...
  const seedRecipesByCluster = (() => {
    ...
    for (const r of filtered) {
      ...
      const m = r.id.match(/^seed-v2-([a-z-]+?)-[a-z0-9-]+$/);
      const slug = m?.[1] ?? null;
      // Multi-word cluster slugs (e.g. "healthy-bowls"): the regex
      // greedily grabs everything between the prefix and the recipe
      // slug. The seed-id convention guarantees the recipe slug is
      // the trailing dash-separated chunk; everything before it is
      // the cluster slug. Fall back to a slow lookup if the regex
      // misses (e.g. id format changed).
      let clusterId: SeedCuisineCluster | null =
        slug && SEED_CLUSTERS.some((c) => c.id === (slug as SeedCuisineCluster))
          ? (slug as SeedCuisineCluster)
          : null;
      if (!clusterId) {
        // Fallback: find the longest cluster id that the recipe id
        // starts with after the prefix.
    ...
    <View style={{ flex: 1, backgroundColor: colors.background, paddingTop: insets.top }}>
      ...
      >
        ...
        {loading && filtered.length === 0 ? (
          ...
          <>
            {/* Wave 4 (2026-05-01) — cuisine cluster carousels.
