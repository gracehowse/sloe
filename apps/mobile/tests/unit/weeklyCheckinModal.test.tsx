 ...
 * ritual (PR claude/weekly-checkin-ritual, 2026-05-02). Gating + content
 * build are unit-covered in `tests/unit/weeklyCheckin.test.ts` (root
 * tests dir) — these tests pin the rendered output:
 *
...
function makeContent(overrides: Partial<WeeklyCheckinContent> = {}): WeeklyCheckinContent {
  return {
    tdeeDeltaKcal: 200,
  ...
  it("renders previous target struck-through alongside the bold suggested target", () => {
    ...
    const tabularStyle = styleArr.find(
      (s: unknown): s is { fontVariant?: string[] } =>
        Boolean(s) && typeof s === "object" && Array.isArray((s as { fontVariant?: unknown }).fontVariant),
    );
