/**
 * CoachScreenView (mobile) — "What to eat next" empty-state branches
 * (ENG-1294).
 *
 * `assembleCandidates` returns `[]` both when the user is over budget and
 * when there is genuinely nothing to rank. The view must branch on the
 ...
 * (web mirror: `tests/unit/coachScreenEmptyState.test.tsx`).
 */
...
describe("CoachScreenView empty states (ENG-1294)", () => {
  it("shows the over-budget/day-done copy when the library has recipes but remaining ≤ 0", () => {
    ...
    expect(screen.getByText(COACH_EMPTY_OVER_BUDGET_COPY)).toBeTruthy();
  ...
  it("keeps the saved-recipes copy for the genuinely-no-recipes case", () => {
    ...
    expect(screen.getByText(COACH_EMPTY_NO_RECIPES_COPY)).toBeTruthy();
