...
- [ ] **[SP customer-lens] T8 — Re-permission audit.** Before shipping content-specific pushes, check whether users who granted permission for the previous generic-body pushes need a fresh permission moment. Planner said NOT needed because the generic body only fired ≤2 times since 2026-04-18; `customer-lens` agent needs to confirm.
- [ ] **[E] T9 — Docs + cleanup.** Docs updated 2026-04-19 (`docs/testflight-feedback/resolved.md` + `docs/journeys/progress.md`). Remaining: remove stale TODO comment at `apps/mobile/app/(tabs)/progress.tsx:628-632` (delivery listener — separate scope), grep for lingering generic-body string literals.
