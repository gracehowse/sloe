...
- [ ] **2026-05-18 — Retire legacy weekly-recap analytics events.** Before this date, fix the `weekKey` off-by-one bug in `weekly_recap_push_sent` and `weekly_recap_push_scheduled` — both currently use `currentWeekKey` (the *new* week we're in) but the recap describes the *previous* week, so funnel joins with `weekly_recap_push_opened` don't line up. This blocks retiring the legacy event names in the 30-day rename-cycle. Route via `analytics-engineer` then `code-quality`.
...
- [ ] **[SP journey-architect] T10 — Confirm Sunday cron timing is user-local.** Customer-lens green-light criterion. Current cron in `vercel.json` fires on a UTC schedule; need to verify it lands at a humane local time per user (not 3am for overseas TZs). If not, route to executor to gate delivery by the user's IANA TZ.
- [ ] **[E] T11 — Mobile-local fallback still carries legacy generic body.** `apps/mobile/lib/weeklyRecapPush.ts:134` fires "Tap to see your weekly recap — avg calories, protein, streak, and weight trend." for installs without a synced Expo push token. Either: (a) promote to a compact mobile-local formatter using cached mobile-side data (nontrivial — no per-user server compute available), or (b) stop mobile-local scheduling entirely and rely on server cron (cleaner; accepts that unsynced installs get no push, which matches the "generic push = worse than no push" principle). Route via `product-lead` for a/b call.
...
- [ ] **[E]** `weekly_recap_push_sent` / `weekly_recap_push_scheduled` `weekKey` off-by-one — see [Hard deadlines](#hard-deadlines) 2026-05-18.
...
- [ ] **🟠 P0-1 — Push token is never actually registered.** ASC `AOjQg5DGBZqS5qNJ1Rqu960`, `APdpODtJDL8q2JhtGup6DK0`. Today `apps/mobile/app/notifications-prompt.tsx` calls `requestPermissionsAsync` but does nothing with the result — no `getExpoPushTokenAsync` call anywhere in the app. Users grant permission, the OS accepts it, but the server never learns their device address so no push is ever delivered. Fix: on grant, call the token API, persist to `profiles.expo_push_token` (needs a migration), set an AsyncStorage flag so the prompt never re-shows. Server uses the stored token in the weekly-recap fan-out above. Owner: `executor`. Review: `qa-lead`. **Blocks the whole weekly-recap push feature.**
...
- [ ] **🟠 P0-3 — Goal calorie target differs between web and mobile for the same user.** `src/lib/nutrition/tdee.ts:calculateBudget` (web) applies a pace-aware deficit — 275 / 550 / 825 / 1100 kcal for relaxed / steady / accelerated / vigorous. `apps/mobile/lib/calcTargets.ts:goalCalorieAdjustment` (mobile) ignores pace entirely and applies a flat −500 / 0 / +300 kcal. Same account on two devices = two different calorie targets. Fix: align mobile to the shared pace-aware table. Owner: `data-integrity`. Review: `nutrition-engine`, `qa-lead`. **Violates the non-negotiable web/mobile parity rule.**
