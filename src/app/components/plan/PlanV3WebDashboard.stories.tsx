 ...
 * prototype `WebPlan`). Two-column: the whole week stacked on the left, a
 * grounded "This week" insight + shopping rail on the right. Pinned states for
 * Chromatic: a populated mixed week and a fresh (all-empty) week. Rendered in a
 * desktop-width frame so the `lg` two-column layout is exercised.
 */
const meal = (
  name: string,
  calories: number,
...
function DesktopFrame({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ width: 1120, background: "var(--bg)", padding: 32 }}>
      {children}
    </div>
  );
}
...
const meta = {
  title: "Plan/PlanV3WebDashboard",
  component: PlanV3WebDashboard,
  tags: ["ai-generated"],
  parameters: { layout: "fullscreen", viewport: { defaultViewport: "responsive" } },
  decorators: [(Story) => <DesktopFrame><Story /></DesktopFrame>],
  args: {
    plan: week,
    targetKcal: 1830,
    weekDates,
...
} satisfies Meta<typeof PlanV3WebDashboard>;
