...
export const DiscoverFeed = memo(function DiscoverFeed({
  ...
  // Wave 4 (2026-05-01) — group seed entries by cluster for the
  ...
  // grouping fights the active query).
  ...
  return (
    <div className="max-w-lg mx-auto min-h-screen bg-background pb-12 md:max-w-6xl md:px-pm-5">
      ...
      <div className="px-0 sm:px-2 md:px-0">
        ...
        {/* Wave 4 (2026-05-01) — cuisine cluster carousels. Five
            ...
            layout still renders for any community uploads (`nonSeedRecipes`)
            so community content is never hidden behind seeds. */}
        ...
        {/* 2026-04-20 desktop prototype port
            ...
            Wave 4: when cluster carousels render above, the flat
            grid + 3-section layout below shows only community
            uploads (`displayRecipes` = `nonSeedRecipes`) so seeds
            aren't duplicated. */}
        ...
        {/* Section 1 + 2: recipe sections — only when there's content
            ...
            Wave 4: reads `displayRecipes` (community uploads only
            when cluster carousels are active above). */}
        {displayRecipes.length > 0 ? (
