...
export const STEP_IDS = [
  ...
  "data-bridges", // 14 — bring your data with you (Build-40)
  "upgrade", // 15 — optional skippable Pro trial decision (ENG-1241) — auto-skipped when `onboarding_conversion_funnel_v1` is OFF
  "first-log", // 16 — guided first-win log prompt (ENG-1233) — terminal when conversion funnel ON
] as const;
 ...
 *   - "trial"  — user tapped Start trial (routes to paywall; may complete onboarding)
 *   - "free"   — user chose Continue on Free
 *   - null     — hasn't answered / step skipped (flag OFF)
 */
export type TrialChoice = "trial" | "free" | null;

/**
 * ENG-1233 — guided first-log chip on the conversion-funnel terminal step.
 *   - "breakfast" | "coffee" | "search" — chip tapped
 *   - "skip"     — advanced without logging
 *   - null       — hasn't answered / step skipped (flag OFF)
 */
...
export interface OnboardingState {
   ...
   */
  appChoice: AppChoice;
  /** ENG-1241 — Pro trial vs free decision when conversion funnel is ON. */
...
export function resolveNextStep(
  ...
  const shouldSkip = (index: number): boolean => {
    ...
    return skipPace || skipAppChoice || skipConversionFunnel;
  };
  ...
  // Overshot because trailing steps are all skipped (e.g. forward from
  // `data-bridges` when the conversion funnel is OFF) — stay put.
 ...
 * the user can reach. Conditionally-hidden steps:
 *  - `app-choice` (gated by `onboarding-app-choice`)
 *  - `upgrade` + `first-log` (gated by `onboarding_conversion_funnel_v1`, ENG-1233/1241)
 * When a flag is OFF the step is removed from the count.
 *
...
export function canAdvance(
  ...
  switch (stepId) {
    ...
    case "reveal":
      ...
      return true;
    case "data-bridges":
      // Build-40 (2026-05-01) — terminal when conversion funnel OFF.
      // When funnel ON, advance continues to upgrade → first-log.
      return true;
    case "upgrade":
