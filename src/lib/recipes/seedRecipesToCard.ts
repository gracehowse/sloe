...
export interface SeedRecipeCard {
  ...
  /** Optional dietary / convenience tags carried through from the
   *  seed (e.g. "high-protein", "vegetarian"). Surfaced on cards
   *  downstream when rendering UI calls for it. */
 ...
 * pinned by `tests/unit/discoverSeedCopyright.test.ts`.
 */
export function seedToRecipeCard(seed: SeedRecipe): SeedRecipeCard {
  ...
  return {
    ...
    creatorName: seed.attribution.author,
