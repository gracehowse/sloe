...
export function parseMfpCsv(input: string): {
  ...
  for (let i = headerLineIdx + 1; i < lines.length; i++) {
    ...
    rows.push({
      ...
      name,
      calories:
        idx.calories !== undefined ? parseNumberCell(cells[idx.calories]) : null,
      protein:
        idx.protein !== undefined ? parseNumberCell(cells[idx.protein]) : null,
      carbs: idx.carbs !== undefined ? parseNumberCell(cells[idx.carbs]) : null,
      fat: idx.fat !== undefined ? parseNumberCell(cells[idx.fat]) : null,
      sodium:
        idx.sodium !== undefined ? parseNumberCell(cells[idx.sodium]) : null,
 ...
 */
export function mapMfpMealToSlot(
  raw: string,
