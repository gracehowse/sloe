/**
 * ENG-855 — `plan_distribute_anchor_v1` registration parity (web ↔ mobile).
 *
 * Make-anything-fit Mode B (Plan-tab distribute-around-anchor) ships DEFAULT-ON
 * per ENG-1279 ("always flag on" so the solo tester sees her own features). It
 * is gated by membership in `REDESIGN_DEFAULT_ON`, which must be present on BOTH
 * `src/lib/analytics/track.ts` and `apps/mobile/lib/analytics.ts` — a flag
 * registered on one side only would render on one platform and stay dark on the
 * other (mirror of the ENG-953 expenditure-trend parity guard). It must NOT also
 * appear in either `KNOWN_DEFAULT_OFF_FLAGS` — a flag belongs to exactly one
 * default set.
 */
...
const FLAG = "plan_distribute_anchor_v1";
...
describe("plan_distribute_anchor_v1 flag registration (ENG-855, default-ON)", () => {
