...
describe("FoodHistoryItem.imageUrl propagation", () => {
  it("surfaces imageUrl on the row when the journal row carries one", () => {
    const byDay: Record<string, M[]> = {
      "2026-04-13": [
        {
    ...
    const out = computeRecentMeals(byDay, 1);
    expect(out[0]?.imageUrl).toBe("https://example.com/sheet-pan.jpg");
  });
  ...
  it("falls back to imageUrl when recipeImageUrl is absent", () => {
    ...
    const out = computeRecentMeals(byDay, 1);
    expect(out[0]?.imageUrl).toBe("https://example.com/pad-thai.jpg");
  });
