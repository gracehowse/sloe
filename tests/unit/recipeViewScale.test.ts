/**
 * Recipe-detail viewing-servings stepper helpers.
 *
 * Pure-helper coverage for `src/lib/nutrition/recipeViewScale.ts`.
 * The DOM-level interaction (stepper +/- on the screen) is exercised
 * separately by the screen tests; this file locks the bounds, clamp,
 * and multiplier maths so PR1's contract ("ingredient grams scale
 * proportionally; min 1, max 99") cannot regress without a test
 * failure.
 */
...
describe("recipeViewScale — constants", () => {
  ...
  it("debounce is 200ms (matches web; mobile should use the same value)", () => {
