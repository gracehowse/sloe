...
describe("householdMemberAccent", () => {
  it("returns the prototype palette for indices 0..3 verbatim", () => {
    expect(householdMemberAccent(0)).toBe("#6c8cff");
  ...
  it("never returns undefined — wraps for >=8 and returns blue for negative / NaN", () => {
    expect(householdMemberAccent(8)).toBe("#6c8cff"); // wraps to index 0 of primary
    expect(householdMemberAccent(-1)).toBe("#6c8cff");
    expect(householdMemberAccent(Number.NaN)).toBe("#6c8cff");
  });
});
