 ...
 * against the product's warm-oat (`--background: #F3F0E8`) canvas. Under
 ...
 * dialog alive, so the change ships behind a flag with the old path in the
 * `else` (CLAUDE.md non-negotiable).
 *
...
describe("ENG-821 — recipe/ingredient editor dialogs token sweep (web)", () => {
  for (const [name, rel] of DIALOGS) {
    describe(name, () => {
      ...
      it("gates the surface behind design_system_elevation (old path in the else)", () => {
        const s = src();
        expect(s).toMatch(/isFeatureEnabled\("design_system_elevation"\)/);
        // flag-ON → warm cream + soft shadow; flag-OFF → today's white/hairline.
        expect(s).toMatch(/bg-background[^"]*shadow-\[var\(--elev-card-soft\)\]/);
        expect(s).toMatch(/bg-card border-border/);
      });

      it("flag-ON surface drops the border-as-depth", () => {
