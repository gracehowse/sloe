...
describe("ENG-1051 — shopping list pantry staple wiring", () => {
  ...
  const webRow = read("src/app/components/shopping/ShoppingListRow.tsx");
  const mobile = read("apps/mobile/app/shopping.tsx");
  const mobileRow = read("apps/mobile/components/shopping/ShoppingListGroupRow.tsx");
  ...
  it("web shopping rows expose mark-as-staple using shared pantry helpers", () => {
    ...
    expect(webRow).toContain('data-testid={`shopping-row-staple-${group.key}`}');
  });
  ...
  it("mobile shopping rows expose swipe + long-press staple actions", () => {
    ...
    expect(mobileRow).toContain('testID={`shopping-swipe-staple-${group.key}`}');
    expect(mobileRow).toContain("Always on hand");
  });
});
