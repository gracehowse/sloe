 ...
 *
 * If any of these regress, the import surface either silently
 * drops user data (worse) or silently invents 0-macro rows
 * (worse still). Both violate CLAUDE.md "if matching is uncertain,
 * do not guess".
 */
...
import {
  ...
  parseMfpDate,
  mapMfpMealToSlot,
} from "../../src/lib/imports/parseMfpCsv";
...
describe("parseMfpCsv — happy path", () => {
  ...
  it("preserves 3 unique dates and 4 distinct meal labels in distribution", () => {
    const { rows } = parseMfpCsv(TEN_ROW_FIXTURE);
...
describe("parseMfpCsv — edge cases", () => {
  ...
  it("returns empty_file warning on an empty input", () => {
  ...
  it("returns missing_required_columns when Date or Food is absent", () => {
    const csv = ["Calories,Carbs,Fat,Protein", "300,30,10,40"].join("\n");
    const { rows, warnings } = parseMfpCsv(csv);
    expect(rows).toEqual([]);
