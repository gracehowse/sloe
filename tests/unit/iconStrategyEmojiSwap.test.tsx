/**
 * Icon-strategy emoji → Lucide swap (P5 parity worklist gaps #10 / #23 / #24;
 * ENG-816).
 *
 ...
 * shipped emoji as functional UI glyphs and were swapped to Lucide:
 ...
 *
 * All three are now UNGATED. The Hydration and CreatorRecipeList swaps were
 * originally gated behind `design_system_icons` with the emoji kept alive in
 * the `else` (CLAUDE.md feature-flag rule); the flag was permanently ON via
 * REDESIGN_DEFAULT_ON on both platforms and was collapsed out entirely
 * (ENG-1651) — the Lucide path now ships unconditionally and the legacy
 * emoji branches no longer exist. The PhotoLogDialog "Plate total" arrow was
 * ungated from the start (ENG-816 #24) to match mobile PhotoLogSheet, which
 * has rendered ArrowRight unconditionally since 2026-05-06.
 *
 ...
 * fallback); mobile never had a live `design_system_icons` conditional of
 * its own for HydrationStimulantsCard (see ENG-816).
 */
...
// Flag gate for PhotoLogDialog's `log_refine_describe_v1` (unrelated to the
// icon swap, which is unconditional post-ENG-1651); default OFF.
...
describe("HydrationStimulantsCard icons (gap #10)", () => {
  it("renders Droplet / Coffee / Wine lucide glyphs and no emoji (unconditional, ENG-1651)", () => {
    ...
    // Functional emoji must be gone — the legacy branch no longer exists.
    expect(container.textContent).not.toContain("💧");
    expect(container.textContent).not.toContain("☕");
    expect(container.textContent).not.toContain("🍷");
  });

  it("each glyph is tinted with its tone colour token", () => {
    const { container } = render(<HydrationStimulantsCard {...HYDRATION_PROPS} />);
...
describe("CreatorRecipeList missing-thumbnail (gap #24)", () => {
  ...
  it("renders the canonical RecipeHeroFallback (no 🍳 emoji, unconditional, ENG-1651)", () => {
...
describe("PhotoLogDialog plate-total glyph (gap #23)", () => {
  ...
  it("Plate total banner leads with a lucide ArrowRight, not 👉 (ungated, mobile parity)", async () => {
