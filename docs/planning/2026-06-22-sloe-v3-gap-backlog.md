...
| 13 | Plan | Web Plan is pre-v3: no two-column grid + Smart/Shopping rail + week-health stat strip + verdict-dot | High | Yes | `MealPlanner.tsx` | `app/(tabs)/planner.tsx` | Build the two-column w-grid + rail (web-only) + stat strip behind `sloe_v3_plan`. Web hero stays carded. |
