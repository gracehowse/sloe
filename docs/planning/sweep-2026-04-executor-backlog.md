...
**Known debt — Supabase:** migration **history vs prod schema** is out of sync (~14+ versions pending on remote after `20260418100000`; some objects missing in prod despite “applied” history). **Planned reconciliation required before the next significant feature release.** Single source of truth: [`docs/planning/supabase-migration-drift-inventory.md`](./supabase-migration-drift-inventory.md) (initial inventory `d3cdc3d`, push/realtime notes `61e4425`).
...
| D1 | **Supabase migration drift vs prod** | Local repo migrations ahead of remote; prod missing objects some migrations assume (`creator_publish_notifications`, etc.) → **silent app failures**. | **Reconcile:** follow [`supabase-migration-drift-inventory.md`](./supabase-migration-drift-inventory.md) — optional `ensure_*` SQL in dashboard, then `supabase db push --linked` (or consolidated catch-up SQL); align `migration repair` with what actually ran. | L | — | supabase | `supabase migration list --linked`: every version has Remote set; smoke nutrition/settings paths that touched hotfixed columns. | executor | data-integrity |
...
G-ANALYTICS — event-name + firing-point portion shipped 2026-04-18 (H5). Dashboards still open.
```

**Quick wins:** G17, DOC-DEBT (both XS; one sitting). G9 is also XS but depends on G8.

**Open decision:** ~~G-ANALYTICS blocked on product-lead event-schema call.~~ Resolved 2026-04-18: event names + payload shape decided (`week_start_day_changed { from, to }`; `fit_this_in_previewed { fromSlot?, overCalories, kcalDelta }`) and instrumented on both platforms. PostHog dashboard wiring remains open for `analytics-engineer`.
