...
| T12 | `VERIFY_STRICT` on release | `verify:production-env` always exits 0 in CI. | Enable `VERIFY_STRICT=1` on `main` or release workflow only. | S | — | CI | **Partial (corrected 2026-05-27, ENG-668):** the script now exits non-zero when `VERIFY_STRICT=1` OR `VERCEL_ENV=production`, but it is NOT yet wired into any automated deploy step (CI sets `VERIFY_STRICT=0`; Vercel `next build` has no `prebuild` hook). The real prod protection is the request-time fail-closed in `rateLimit.ts` (ENG-668). Wiring a prod `prebuild`/CI gate is the remaining follow-up. | executor | release-gate, security-reviewer |
