...
| Phase 3 / 5 | B2.3 — Onboarding produces first plan | **Shipped** (2026-04-27 Phase 5); **picker superseded 2026-05-30** | Phase 3 selection state + threshold parity → Phase 5 added `<RecipePickerGrid>` web + mobile, 15 hand-picked seed JSON, `onboardingSeedResolver` (title-match), `buildFirstWeekFromSeeds` (planner + `save_meal_plan` RPC), seed migration staged at `20260503110000_onboarding_seed_recipes.sql`. The picker step (`recipes`) and its grid were cut 2026-05-30 as dormant code — see `docs/decisions/2026-05-30-cut-onboarding-recipe-picker.md`; the library now seeds automatically via `selectOnboardingSeeds`, so "first plan" still ships on completion, just without a user-facing pick step. Current journey doc: `docs/journeys/onboarding-to-first-log.md` (the old `onboarding-final-step-2026-04-27.md` is now a redirect stub). |
...
| Phase 4 / 5 | B3.2 — Coeliac/gluten depth | **Shipped — chip + classifier** (2026-04-27 Phase 5) | `glutenClassifier.ts` (15 grains + 8 implicit dishes + 4 risk markers + ~70 naturally-GF foods, word-boundary regex), `classifyRecipeGluten` aggregator, `gluten-high-conf` / `gluten-uncertain` chip rendered on web + mobile recipe detail. 43 classifier tests + 4 mobile parity tests. Library/Discover filter pill + inline ingredient `Wheat` glyph deferred behind legal-reviewer copy review. Journey at `docs/journeys/gluten-depth-2026-04-27.md`. |
| Phase 4 | B3.3 — TestFlight expansion (open beyond N=1) | **Open** | Gated on demo moment shipping (B2.1–2.4) |
| Phase 5 | B3.M — Mobile NorthStarBlockHost wiring + Verify parity | **Shipped** (2026-04-27) | `NorthStarBlockHost.tsx` mobile component (mirrors web), wired in `(tabs)/index.tsx` between TodayHero and macro tiles. Per-day skip ledger via AsyncStorage `suppr.northstar.skipped.<dateKey>`. V-5 closed: inline "Verify →" text-button on estimated ingredient rows in mobile recipe detail (`recipe/[id].tsx`). |
| Phase 5 | B5 — Shared-element transitions | **Partial** (2026-04-27) | Web view-transition-name annotations on Library card + RecipeDetail hero (browsers with VT API morph automatically). Mobile shared-element morph **deferred** — Reanimated 4 dropped `sharedTransitionTag`; choice between v3 pin / native-stack screenTransition / layout-prop morph staged for follow-up. Journey at `docs/journeys/recipe-transitions-2026-04-27.md`. |
...
|---|---|---|
| Import — stop verbatim creator prose (web/blog) + disclaimer | **Shipped** | ENG-857 launch-blocker. SHIPPED 2026-06-11 (audit P0-2 closed): the web/blog + social-website-fallback import branches now null `description` at the route boundary (`route.ts` web-scrape spread override + social-fallback branch), so neither persist layer (`saveImportedRecipe.ts`, web editor form) can receive the prose for non-first-party imports. `parsed.description` is still fed to `extractCaptionNutrition` for the macro-sanity check only (server-side; never surfaced). First-party recipes keep their own description. Tests: `tests/integration/recipeImportDescriptionNull.test.ts`. |
| Import — surface link-back + creator credit + disclaimer | **Shipped** | ENG-858 / ENG-1042 launch-blocker. SHIPPED 2026-06-11 (audit P1-7 closed): imported (non-first-party) recipes render the legal-approved attribution + disclaimer on both platforms (`RecipeDetail.tsx` `recipe-import-disclaimer`; `recipe/[id].tsx` source-card disclaimer), gated on `isImportedRecipe`, wording from the single shared constant `src/lib/recipes/importSourceDisclaimer.ts`. Tests: `tests/unit/importSourceDisclaimer.test.ts`, `recipeSourceCardParity.test.ts`. DMCA agent (ENG-859) remains open (incorporation-dependent, Grace-owned). |
| Import — route all fetches through the SSRF guard | **Shipped** | ENG-1037 launch-blocker (audit P1-2). SHIPPED 2026-06-11: every recipe-import fetch on a user-supplied/derived URL now routes through `followWithSsrfGuard` (per-hop allowlist + DNS re-resolve) — the caption-link Tier-4 fallback + the social-meta UA loop + the social image fetch (all were raw `redirect:"follow"`/default-follow) and the main hop loop (added DNS re-validation). CI grep guard bans `redirect:"follow"` in the import surface. Tests: `tests/unit/recipeImportSsrfGuardCallsites.test.ts`, `ssrfGuard.test.ts`. |
