...
- The `onboarding_v2_redirect_followed` PostHog event gives an EOL
  signal. If hit-count drops to ~0 across a 14-day window before
  2026-07-31, the redirect and route file can be deleted ahead of
  schedule.
- After removal, `/onboarding-v2` 404s naturally via Expo Router
  NotFound fall-through — the desired end state.
