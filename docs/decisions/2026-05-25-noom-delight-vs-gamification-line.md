...
## Clarification (2026-05-30) — gamification is allowed; streak-freeze is in-bounds

The 2026-05-30 full-sweep re-flagged the streak-**freeze** ledger (`computeProtectedStreak`, `streak_freeze_budget_max`, `streak_freezes_used_history`; `src/lib/nutrition/streakFreeze.ts`) as a possible gamification-line violation. **It is not — Grace confirmed "yes we allow gamification" (2026-05-30).** Run it through the gate: a freeze *absorbs* a missed day so the streak does not break — it **removes** punishment, it does not add it. That is the opposite of loss-aversion, so it passes ("does it punish a missed day? → no"). It is the same posture as the freeze-protected shield glyph shipped under DC8 (ENG-234). This is an intentional, in-bounds mechanic — **not a gap.** Do not re-open it in future sweeps without a new product call from Grace.
