...
- **Today / Log-sheet call-sites + the aubergine-outline guard repoint** — owned by the Today + Log-sheet migration lanes; each repoints its slice of `{apps/mobile/tests/unit/todayLaneAubergineOutline.test.ts,tests/unit/todayLaneAubergineOutlineWeb.test.tsx}` as it lands its component change.
  - **Today modal/banner holdouts** — migrated both platforms (the last outline holdouts on Today): complete-day modal "View my progress" → **primary** (the sheet's sole CTA); weekly-checkin modal "Accept new target" → **primary** + "Keep current" → **ghost**; weekly-checkin banner "OPEN" → **ghost** (nudge → opens the modal; mobile-only — web surfaces the Digest on Progress); eat-again "Log it" → **ghost** (nudge; north-star hero owns the primary log moment, the × dismiss stays icon-only). Files: `apps/mobile/components/today/{TodayCompleteDayModal,WeeklyCheckinModal,WeeklyCheckinBanner,TodayEatAgainBanner}.tsx` + web twins `src/app/components/suppr/{today-complete-day-dialog,weekly-checkin-dialog,today-eat-again-banner}.tsx`. Guard pins repointed in both `todayLaneAubergineOutline*` files.
