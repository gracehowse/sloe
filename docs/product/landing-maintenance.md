...
| AI photo meal recognition | Server-gated Pro (`tier !== "pro"` → 403); 100/day Pro rate limit. | Pro | Base + Free loophole closed 2026-04-19 (same-day as voice-log) — `POST /api/nutrition/photo-log` now rejects non-Pro tiers with `upgrade_required`, mirroring the client gates and the Pro-only bullet "AI photo meal recognition (100/day)". |
