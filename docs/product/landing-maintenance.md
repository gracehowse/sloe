...
| AI photo meal recognition | **Free taster** (3/day for Free + Base via `api:photo-log:free-quota` bucket; 4th attempt → 403 `upgrade_required`); Pro 100/day via the existing `api:photo-log` bucket. | Free taster + Pro | 2026-05-02 — flipped from Pro-only to "3 free taps/day before paywall" per the Cal AI convert-hook call. Decision doc: [`docs/decisions/2026-05-02-photo-log-free-taster.md`](../decisions/2026-05-02-photo-log-free-taster.md). The landing copy line "AI photo meal recognition (100/day)" still speaks accurately to Pro; the free taster is intentionally not advertised on the landing (it's an in-flow surprise, not a planned feature). |
