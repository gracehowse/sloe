...
### A4 — Per-piece count reference weights + confidence gating — FIXED (ENG-1544, 2026-07-12; superseded by A5)
...
- **Fix (as shipped 2026-07-12 — field name changed under A5, see below):** `foodSpecificCountRef(name)` returned `{ grams, confident: boolean }`. Foods with a defensible **single-piece USDA FoodData Central / Handbook** reference weight were `confident: true`: almond 1.2 g, walnut half 2.5 g, cashew 1.5 g, pistachio kernel 0.7 g, hazelnut 1.0 g, pecan half 1.0 g, macadamia 2.5 g, peanut 1.0 g; fig fresh 50 g / dried 8 g; apricot fresh 35 g / dried 8 g; plum 65 g; prune 9.5 g; date 7 g; button mushroom 20 g; strawberry 12 g (plus the existing meat cuts and medium-produce references). Two catch-all buckets — misc pickled/allium bits (olive/caper/cornichon/gherkin/radish/**shallot** ≈ 5 g) and misc small shellfish (prawn/shrimp/mussel/clam/scallop/oyster ≈ 15 g) — still returned a weight so downstream math worked, but were `confident: false`. `measureToGramsConfidence` rated a count/size word `"high"` only when `confident === true`; anything else was `"low"`.
- **Status:** **SUPERSEDED by A5** — the binary confident/not-confident shape below no longer exists in code; kept here for history since ENG-1544 was the first fix in this lineage.

### A5 — Split the widest-range buckets + add a MEDIUM confidence tier — FIXED (ENG-1432/count-to-weight-3, 2026-07-20)

- **File:** [`src/lib/nutrition/measureToGrams.ts`](../../src/lib/nutrition/measureToGrams.ts)
- **Was:** the 2026-07-05 audit re-flagged A4's remaining shape as still too coarse in two ways:
  1. **Shallot** shared the misc pickled/allium 5 g bucket with capers (0.3 g) and gherkins (10 g) — a real shallot bulb is ~25-35 g, ~5-7× the bucket's number.
  2. **Any mushroom** — including "portobello" — resolved to the flat 20 g button-mushroom weight at `confident: true` (HIGH). A large portobello cap is ~80-150 g, so "1 large portobello" still landed on a 4-7× undercount at HIGH confidence.
  3. The binary `confident: boolean` collapsed "a real food-specific catch-all match, just imprecise" (olive/shrimp bucket) and "no food-specific rule at all" (a genuinely unknown ingredient) into the same `"low"` tier at the `measureToGramsConfidence` level — the coarseness of the former was invisible.
- **Fix:** `CountPieceRef.confident: boolean` → `CountPieceRef.confidence: "high" | "medium"`, and `measureToGramsConfidence` returns `"high" | "medium" | "low"` (was `"high" | "low"`). Shallot (25 g) and portobello/portabella (100 g) were split out of their catch-alls into their own single-food HIGH references, same treatment as onion/potato/tomato. The two catch-alls left behind (misc pickled/allium minus shallot; misc shellfish) now read `"medium"` — a real food-specific class matched, but per-piece weight varies too widely across the *different* foods in the bucket to trust as a specific value. A food with **no** rule at all (e.g. an unrecognised ingredient falling to the generic 80/110/180 g size fallback) still reads `"low"`. The sole caller, `tryCountToWeightGrams` in [`shoppingMergePrimitives.ts`](../../src/lib/planning/shoppingMergePrimitives.ts), gates on `!== "high"`, so `"medium"` is refused identically to `"low"` today — the three-tier split adds a real, extendable signal for future display without loosening the never-guess-a-weight contract.
- **Status:** **FIXED.** Pinned in `tests/unit/measureToGrams.test.ts` (shallot → 25 g HIGH; portobello/portabella → 100 g HIGH, cremini unaffected at 20 g; olive/shrimp → MEDIUM, not HIGH; an unmatched food returns no ref at all) and `tests/unit/measureToGramsConfidence.test.ts` (shallot + portobello → HIGH; olive/caper/cornichon/shrimp/mussel/scallop → MEDIUM; MEDIUM still fails the `!== "high"` cross-convert gate the same as LOW).
