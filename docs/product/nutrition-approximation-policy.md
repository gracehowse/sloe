...
- **Fix:**
  - Single tunable accept floor `MIN_ACCEPT_CONFIDENCE = 0.70`. `MIN_MATCH_CONFIDENCE = 0.70`; `MIN_OFF_CONFIDENCE = 0.72` (one notch stricter for noisy product names).
...
- **⚠️ NUTRITION-ENGINE IMPACT REVIEW REQUIRED BEFORE MERGE.** Raising the floor to 0.70 risks over-rejecting common foods that legitimately score in the old 0.42–0.70 band. Measured examples on the current scorer that now FAIL 0.70: "brown rice" (0.475), "whole milk" vs "Whole milk, 3.25%" (0.662), "tomatoes canned" (0.433), "all purpose flour" (0.283), "salmon atlantic raw" (0.325). (The real pipeline applies `applyNameAliases` first, so production scores differ — but the over-rejection risk is real and unquantified without modeling against real recipes.) Tracked as a canary in `tests/unit/confidenceGating.test.ts` ("KNOWN over-rejection: 'brown rice'…").
