...
- "Eat again" — re-logging a prior-day meal in one tap. The dedicated Today banner was **retired from both platforms** in the Sloe Figma 654:2 Today unification (mobile 2026-05-22; web commit 664df1cb) so Today shows at most the north-star / deficit prompt. Re-logging now lives in the Quick Add picker's Recent / Frequent tabs. The `computeEatAgainForSlot` helper (`src/lib/nutrition/foodHistory.ts`) and the `TodayEatAgainBanner` component are retained and test-pinned on both platforms but are not mounted on Today.
