...
| Import / capture | URL recipe import; ingredient match to DB | Social + screenshot/handwriting; browser extension | **Both required**: **URL import** and **social/screenshot import**; phase *which networks* first if needed |
...
| Social | Forums, diary sharing | Save from social into app | **Creator feed + follow + save** |
...
Ship **URL recipe import** (parse URL → structured recipe → user confirms) **and** **social/screenshot import** (image or share target → extraction → confirm). Do not treat them as either/or; sequencing can be parallel (e.g. URL first in web, screenshot first on mobile) but **both** are product commitments.
...
Reinforces macro planner, fiber/water/Health, and the creator feed. **Sequencing:** nail **diary + dual import + recipe nutrition** before scaling creator count.
