...
| **Warning** | `#C9892C` (light) / `#D6A24A` (dark) | `Accent.warning` | `--accent-warning` / `--warning` | Approaching limits **and over-budget** (amber re-ratified 2026-07-01, ENG-1296 — the D-2 red carve-out is retired). **Not** activity bonus (that owns honey) |
| **Warning solid (text)** | `#925812` | `Accent.warningSolid` | `--accent-warning-solid` | Amber as small text on light (AA-safe; darkened from `#956619` 2026-06-23). Also the over-budget foreground — `--over-budget-fg` / `overBudgetFg` resolve to this (ENG-1296) |
| **Carbs** | `#C8794E` (light) / `#D58A5E` (dark) | `Accent.carbs` | `--macro-carbs` | Carbs (+ sugar) macro track — Sloe clay (same hue family as primary) |
| **Activity** | `#D6A24A` (light) / `#E0B25E` (dark) | `Accent.activity` | `--activity` | Activity / burn / earned-bonus — Sloe honey, distinct from the darker warning/over-budget ambers. **FILL-ONLY** — honey is 2.3:1 even on white, so it can never be text (any size) |
...
| **Destructive** | `#C0533F` (light) / `#DC6B55` (dark) | `Accent.destructive` | `--accent-destructive` / `--destructive` | Errors and dangerous actions **only** (over-budget is amber since 2026-07-01, ENG-1296) — Sloe warm brick |
...
5. **Over-budget = amber warning family, uniformly (2026-07-01 re-ratification, ENG-1296).** The dossier D-2 "over = destructive red" rule is RETIRED — Grace's sweep decision #2 (`docs/decisions/2026-07-01-sweep-decisions.md`): diet-culture research (red numbers = guilt cycles) outweighs at-a-glance severity. The over ring arc is `--ring-over-a/b` (`#925812` → `#C9892C` — warning-solid → warning); the "kcal over" numeral and every over-budget foreground read `--over-budget-fg` / `overBudgetFg` (= warning-solid). Never re-introduce red for over-budget.
