...
- **Edamam `/nutrients` detail calls aren't quota-counted** — *DOWNGRADED P1→P2; verifier added: same gap on USDA detail.* `app/api/edamam/food/route.ts` has no `consumeQuota('edamam')`; on a viral day the unguarded detail calls can blow the 1,000/day ceiling. Wire `consumeQuota` + a `foodId` cache.
...
- **No retry at any vendor call site; USDA search returns 502 (not the degraded envelope) on failure** — a 1–2s USDA hiccup breaks the whole merge pipeline. Add one bounded retry + return the degraded 200. **RESOLVED for USDA search (ENG-1119, 2026-06-18):** `/api/usda/search` now retries ONCE on a transient 5xx (or thrown network/timeout — never on a 4xx/429) with a 300ms backoff, then degrades to the same 200 envelope shape as the OFF route (`{ ok:true, hits:[], degraded:true, degradedReason:"usda_unavailable" }`) instead of 502. Quota is spent once per logical search; failures aren't cached. Tests: `tests/unit/usdaSearchResilience.test.ts`. *Still open:* the other vendor detail/`food` call sites have no retry yet (lower priority — they're not the merge spine).
