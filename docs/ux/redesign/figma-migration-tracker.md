...
| Progress | 35 | 9 | 17 | 9 | 0 | 492:2 single-path rebuild 2026-06-08 (range toggle + lilac THIS WEEK + AVERAGE ADHERENCE 4-bar + weight Trend/Scale+stat-row+log + AVG/TDEE/DEFICIT triad + sage/amber Daily Calories + on-target ribbon ✅); `progressLayoutV2` flag retired (single path); fibre data gap CLOSED (real `fiber_g` + 28g default) |
...
| Paywall & win moments | 22 | 2 | 4 | 15 | 1 | mobile paywall + web /pricing rebuilt to `284:2` (2026-06-08); streak/import-success still full screens in Figma |
...
**Paywall (4 remaining):** upgrade dialog; M5 streak win (high); M6 import-success (high); trust strip. ✅ 2026-06-08: mobile paywall + web /pricing rebuilt to `284:2` — photo hero + "SLOE PRO / Cook what you love. *Still* reach your goals." headline, 2×2 value grid (Unlimited imports/Macro fitting/AI coach/Cloud sync), FREE/PRO comparison matrix (lilac PRO column), Annual(BEST VALUE, computed Save %, selected clay border)/Monthly plan selector, clay CTA (off-brand violet→indigo gradient removed), trust chips. All copy from a shared SSOT (`src/lib/landing/paywallValueProps.ts`); prices stay resolver-bound (web `PRICING_TIERS`→Stripe; mobile RC `priceString`), VAT-inclusive posture preserved. No feature flag (one production design system).
...
2. **Flag-gating debt** (migration flags to retire later, NOT now): `today_meals_figma_654`, `today-status-pills`, `plan_empty_state_v2`, `plan_source_selector`, onboarding flag, etc. (`progressLayoutV2` **RETIRED 2026-06-08** — the 492:2 layout is the single Progress path, no flag.)
3. **File-size debt** (CLAUDE.md 400-line cap): mobile `(tabs)/index.tsx` 6412L, `NutritionTracker.tsx` 3613L, `SettingsBundleContent.tsx` ~3600L, web `Settings.tsx` ~88KB — structural refactor needed for clean conformance. (mobile `progress.tsx` 4007L→2247L on the 2026-06-08 single-path rebuild; still over cap — further extraction tracked, but halved.)
...
| ENG-925 | Meal-row image | **Pending side-by-side prototype** | Building A/B/C compare + universal-food-imagery (Template-B per common food, cache by food key). Likely → B-becomes-universal |
...
### Progress — full 492:2 single-path rebuild 2026-06-08 (local only, no commit; web+mobile tsc clean; 139 progress tests green; web pixel-verified)
**Supersedes the DS-tightening pass above.** That pass reskinned the existing cards but the screen was still MISSING frame sections and the mobile render was still split across the flag-gated `progressLayoutV2` (ENG-773) layout + a legacy fallback. This pass builds the frame in full and collapses mobile to ONE production path (no flag, no alternate version), matching web.

**Built (both platforms, mirrored):**
  - **Range toggle** `7d/30d/90d/All` — active = plum fill / white (`bg-foreground-brand` web · `t.plum` mobile), bordered cream inactive. Drives the range-scoped stats + weight/calorie windows.
  - **THIS WEEK insight → LILAC** — `ProgressHeadline` (and the `ProgressStoryGate` empty placeholder) moved off cream onto the damson-soft wash (`--slot-dinner-soft` web · `rgba(106,75,122,0.12)` mobile) with the clay sparkle + eyebrow. Shared `PROGRESS_INSIGHT_LILAC_*` constants on mobile so headline + gate never change tone across the data floor.
  - **AVERAGE ADHERENCE card** (new `ProgressAverageAdherence` web + mobile) — big calorie-adherence % + week on-target dot streak + 4 labelled macro bars (Protein sage / Carbs clay / Fat amber / Fibre teal), over-target = amber. Reads range-scoped `buildMacroAdherenceRangeStats` so the bars track the same window as the headline %.
  - **Weight card** — serif kg headline + "↓ N this week" + **Trend/Scale** segmented toggle (smoothed MA vs raw weigh-ins) + clay sparkline/line with dashed goal line + **START / CURRENT / GOAL / RATE** stat row + "＋ Log weight" + View-all-measurements (detailed Withings chart + all-data sheet preserved behind it).
  - **AVG INTAKE / EST. TDEE (ADAPTIVE) / DEFICIT triad** (new `ProgressEnergyTriad`) — three cream cells, sage TDEE/deficit, amber surplus.
  - **DAILY CALORIES** — M–S bars **sage = on target / amber = over** (never red — calorie-ring carve-out is bars-exempt), goal dot above each bar, "On target / Over" legend.
  - **On-target ribbon** (new `ProgressOnTargetRibbon`) — medal glyph + "N on-target days this week" (real count; hides at 0 — no empty achievement). Web `Icons.award` added (lucide `Award`) for parity with mobile.

**Fibre data gap CLOSED:** `buildWeekStats` now rolls up `avgFiber`/`fiberAdherence`; `buildMacroAdherenceRangeStats` added (range-scoped, shared); mobile loads `nutrition_entries.fiber_g` + `profiles.target_fiber_g` (28g shared default = web `normalizeMacroTargets`). The Fibre bar reads a REAL value on both platforms — no fabrication (null fibre rows count as 0).

**Governance:** `progressLayoutV2` flag + the `design_system_elevation`-gated dual render REMOVED — single path (file 4007→2247 lines; dead `WeightRangeCard`/`CaloriesRangeCard`/`pillStyle`/`rangeLabel`/streak-chip derivations deleted). Every wired feature preserved (time-range, adherence calc, weight log + Trend/Scale, adaptive TDEE/maintenance + explainer, deficit, daily-calorie data, measurement history, Apple Health, journey/projection, week digest, empty/zero states). No fabricated data (the week-over-week adherence "↗ up N%" trend chip stays hidden — documented data gap, not faked). Calorie-ring red-over carve-out untouched. No shared token files touched.

**Files:** web — `src/app/components/ProgressDashboard.tsx`, `suppr/progress-average-adherence.tsx` + `progress-energy-triad.tsx` + `progress-ontarget-ribbon.tsx` (new), `suppr/progress-headline.tsx`/`progress-story-gate.tsx` (lilac), `ui/icons.ts` (+award), `lib/nutrition/progressRangeStats.ts` + `progressWeekReport.ts` (fibre + range macro). mobile — `app/(tabs)/progress.tsx` (single-path rewrite + fibre load), `components/progress/ProgressAverageAdherence.tsx` + `ProgressEnergyTriad.tsx` + `ProgressOnTargetRibbon.tsx` (new), `components/today/ProgressHeadline.tsx` + `ProgressStoryGate.tsx` (lilac + sparkle), `components/tabs/ProgressTabChrome.tsx` (overline optional). Tests updated: `tests/unit/progressPhase2Cards.test.ts` (asserts the real component/template structure), `apps/mobile/tests/unit/progressRangePicker.test.ts` (overline retired; subtitle + plum-pill rail).

**Verified:** web mobile-vp `screenshots/web-drive/progress-after.png` — range toggle (30d plum) + lilac THIS WEEK + AVERAGE ADHERENCE 150% with sage Protein / amber Carbs·over / Fat·over bars ✓; element captures — weight card (Trend/Scale + clay line + START 47.3/CURRENT 54.6/GOAL/RATE + Log weight) ✓, energy triad (AVG 1,353 / EST.TDEE 1,394 ADAPTIVE / DEFICIT −41 sage) ✓, daily calories (M–S + goal dots + On target/Over legend) ✓. iOS capture owned by the orchestrator (this agent does not touch the sim). Typecheck clean web+mobile (`tsc --noEmit` exit 0 both); progress tests green — web 114/14 files, mobile 25/5 files. Empty/zero states render the scaffolds (test user <3 days logged → story-gate placeholder + zeroed bars + hidden ribbon, matching the frame's layout).
