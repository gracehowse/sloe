...
|---------|-----|--------------------|
| Marketing → signup (landing, `/pricing`, referral `/g/<code>`, `/roadmap`, `/whats-new`, trust/meta pages) | [marketing-to-signup.md](./marketing-to-signup.md) | Playwright `tests/e2e/screenshots/web-public-routes-after.spec.ts`; Vitest `tests/unit/landingParity.test.tsx`, `tests/unit/referralClient.test.ts`, `tests/unit/referralInviteParity.test.ts` |
