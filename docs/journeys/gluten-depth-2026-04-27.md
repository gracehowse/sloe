...
Legal-reviewer copy review pending pre-App-Store-submission.
...
| Any `risk + medium` | `gluten-uncertain` | `"Gluten contamination risk · review"` |
| All `free + high` | `gluten-high-conf` | `"Gluten-free · high confidence"` |
...
1. **Legal-reviewer copy review** — the spec-pinned strings
   ("Gluten-free · high confidence" / "Gluten contamination risk ·
   review") ship as in-app copy. Pre-App-Store-submission, the
   legal-reviewer must sign off on the wording per coeliac-claim
   guidance (UK FSA + EU 1169/2011 §22). Tracked in Tasks DB.
2. **Library + Discover filter chip** — "Gluten-free" filter pill
   gated on the same legal-reviewer review.
