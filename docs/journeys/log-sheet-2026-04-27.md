...
Updated 2026-04-28 (search-first refactor), 2026-04-30 (inline-search refactor), 2026-05-30 (ENG-773 slot selector), and 2026-06-07 (Sloe DS reskin + S13 logged-confirmation).
...
- **Empty states + CTAs:** Recent / Saved / Library empty blocks and the
  manual-entry + Browse-recipes buttons bumped to the Sloe `rounded-xl`.
...
- A cream item card with the logged title + provenance dot + **"Est. {n}
  ...
  absolute).
- A clay primary **"Done"** (the one primary action) + an optional quiet
  **"Undo"** (rendered only when the host wires `onUndo`).
