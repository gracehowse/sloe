...
- **Per-serving display** (TestFlight `APo0qS9vcFvmBJEJJ_-61YA`, 2026-04-19). Every row that exposes a natural portion (Edamam `servingSizes[]`, USDA Branded `servingSize` + `householdServingFullText`, USDA Survey `foodPortions[]`, parsable OFF `serving_size`) shows `{kcal} kcal · P/C/F` for the portion as the primary line, and a subdued secondary line of the form `{label} ({grams} g) · {per100gKcal} kcal / 100 g`. Rows with no natural portion (generic USDA, Edamam rows that expose only `"Gram"`, OFF free-text like `"1 piece"`) fall back to the /100g-only display unchanged. Inference lives in `src/lib/nutrition/primaryServing.ts` and is imported by both platforms.
