...
| `user_favorite_foods` / `user_saved_meals` / `user_saved_meal_items` / `user_recipe_notes` / `user_custom_foods` / `user_plan_templates` | Per-user private; full CRUD scoped to `auth.uid() = user_id`. **Note (ENG-1420):** manual `user_custom_foods` creation now writes via the service-role `POST /api/custom-foods` route (server-side plausibility gate), scoping `user_id` to the JWT-verified caller; the owner CRUD RLS policies remain as defence-in-depth for the still-authed read/update/delete + photo-correction paths. Extra column `plausibility_overridden boolean not null default false`. | for_all (user_id) |
