...
| **F-149** | (recurrence Theme 3) | 🔄 PR #176 (mobile retune) | P1 numbers | **Smaller-fix path shipped (mobile)**: `backfillDailyTargetsFromProfile` helper added to `src/lib/nutrition/dailyTargetSnapshot.ts`. Wired into `apps/mobile/components/recap/GoalPaceRetuneSheet.tsx` BEFORE the profile update — on retune, reads the current (about-to-be-old) profile and upserts synthetic snapshots for the past 30 days using its values. `upsert(..., { ignoreDuplicates: true })` protects existing snapshots; only gaps get filled. Past days without logs now show the target that was effective on that day, not the new one. **Web parity + Settings → Targets retune path still parked** — that touches `src/app/components/Settings.tsx` and the web onboarding retune; same helper would apply, scope deferred to keep this PR cohesive. **Proper Theme 3 fix (`goal_history` table) still parked** — schema decision worth its own PR. |
...
| **F-152** | `AOOBv-1Ow` (recurrence Theme 7) | 🟠 PR #175 (partial) | P2 polish | **Most-prominent emoji replaced**: locked plan-length chip in `apps/mobile/app/(tabs)/planner.tsx` previously appended `" 🔒"` inside the chip text. Replaced with a real `Lock` lucide icon. **Wider sweep deferred**: project-wide grep for emoji literals in product chrome needs review for which are intentional vs accidental. Follow-up PR. |
