...
| **F-156** | (no specific ID, multiple mentions) | 🟠 PR #184 (PR-1 of 2) | P2 feature | Spec at `docs/planning/F-156-custom-food-parity-spec.md`; 3 open questions resolved by Grace 2026-05-10. **PR-1 shipped (PR #184):** `convertMacrosBetweenBases` helper in `src/lib/nutrition/customFoods.ts` (7 unit tests pin round-trip stability + zero/negative guards) + per-serving / per-100g basis toggle on both `apps/mobile/components/CreateCustomFoodSheet.tsx` and `src/app/components/suppr/create-custom-food-dialog.tsx` + Fibre promoted into the 2x3 macro grid (was a full-width row). Default basis = per-serving when natural serving exists, else per-100g; user's last choice persists via AsyncStorage / localStorage; "Values converted to {basis}" notice on flip. Macros stay stored per-100g internally (no schema change). **PR-2 (TBD):** scan-to-prefill from barcode-failure path + unlimited multi-serving array (Grace overrode the spec's cap of 3). PR-3 (read surface for Cholesterol + Potassium) + PR-4 (write fields + schema) deferred. |
