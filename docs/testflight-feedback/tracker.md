...
| **F-150** | `AKvgjnb` + `APGJJlg` (recurrence Theme 1) + new mentions | 🟠 PR #175 (partial) | P1 portions | **Worst-case macro corruption closed**: `apps/mobile/lib/verifyRecipe.ts:317` previously did `unit === "g" && amount ? amount : 100` for the per-100g back-calc, so any non-gram unit (cup of rice, medium banana, etc.) divided macros by factor=1 and inflated per-100g by up to 58%. Now uses `measureToGrams` from the shared nutrition module, which covers eggs, cups (region-aware), tins, and the COUNT_WEIGHT_G table, so non-gram units resolve to a defensible weight before the back-calc. **Receiver-side fallbacks deferred**: 5 sites where the search panel's resolved `PrimaryServing` is thrown away on selection (`verify.tsx`, `import-shared.tsx`, `create-recipe.tsx`, `CreateRecipeWizard.tsx`, `customFoodsClient.ts`) need a contract change to plumb `PrimaryServing` through the food-pick callback. Sequenced for a follow-up PR. |
| **F-151** | `APdpODtJDL8q2JhtGup6DK0` | ⏳ diagnosis needed | P2 broken | "Notifications don't appear to be working". Code path exists: `apps/mobile/lib/expoPushToken.ts` registers a push token to `profiles.expo_push_token`, `pushNotificationsSetup.ts` + `weeklyRecapPush.ts` consume it. Common failure modes: (a) permission denied + cached "asked" (so `notifications-prompt.tsx` no-ops), (b) token registration silently logs to `console.warn` only, (c) server-side push send not wired or not firing on cron. Needs a focused diagnosis with PostHog instrumentation on each step + a probe push send. Deferred to a focused follow-up because the right fix depends on which step is the actual failure. |
| **F-152** | `AOOBv-1Ow` (recurrence Theme 7) | 🟠 PR #175 (partial) | P2 polish | **Most-prominent emoji surfaced replaced**: locked plan-length chip in `apps/mobile/app/(tabs)/planner.tsx:1903 + 2048` previously appended `" 🔒"` inside the chip text (look like typo, inconsistent across iOS / Android emoji fonts). Replaced with a real `Lock` lucide icon in a flex row next to the label, with `accessibilityLabel="Pro only"`. Theme #7 architectural lesson upheld. **Wider sweep deferred**: a project-wide grep for emoji literals in product chrome (across `apps/mobile/app`, `apps/mobile/components`, `src/app/components`) needs review for which emojis are intentional (macro tile icons, success-card celebrations) vs accidental — not safe as a single search-replace pass. Follow-up PR. |
