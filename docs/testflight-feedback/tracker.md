...
| **F-149** | (recurrence Theme 3) | ⏳ parked | P1 numbers | "Plan update retroactively changes past-day goals". `dailyTargetSnapshot.ts` only writes for `today` (L57). Past days without a snapshot fall back to `currentTargets` in `dailyTargetRead.ts:115-122` — i.e. the live profile target, not the target effective on that day. Two fix paths: (a) backfill `daily_targets` for every past day with previous profile values on goal/pace retune (smaller, safer) or (b) add a `goal_history` table keyed by `(user_id, effective_from)` and resolve by date-bound query (proper fix per Theme 3 architectural lesson). Both are write-path changes worth a focused PR with proper test coverage. F-145's snapshot fix already partly mitigates: past days that DID get snapshotted now have a defensible Maintenance number. |
