...
**Themes:** ~~Plan~~ **TODAY-tab** redesign cohesion + planned-card styling (F-158–F-160, F-177–F-179) — **ENG-1065 re-triaged 2026-06-12**: all five founder screenshots show the TODAY tab active (Complete Day button = `index.tsx`, "Planned" card = `TodayPlannedMealsCard.tsx`, meal sections = `TodayMealsSection.tsx`); F-160's "ugly section" is the white-slab meal cards — converges with the Withings flat-card direction (ENG-1078); design-system drift (rings, buttons, corners, spacing — F-164–F-170, F-174–F-177) — **ENG-1064 triaged**, ring stroke needs design call; Library regression (F-171); search vendor micros display (F-161–F-162); perf (F-168); ingredient switcher regression (F-173); positive benchmark (F-163).
...
| **F-145** | `ALcwMFPjf` + `AAtW7dYcCBP` (recurrences) | ✅ PR #176 + PR #178 | P1 numbers | **Snapshot drift closed (PR #176)**: `dailyTargetSnapshot.ts` resolves maintenance via canonical `resolveMaintenance` at write time. Past-day reads show the same number Today shows. **`getEffectiveTDEE` staleness check shipped (PR #178)**: both web `src/lib/nutrition/tdee.ts` and mobile `apps/mobile/lib/calcTargets.ts` now reject adaptive values older than 14 days when the caller passes `adaptive_tdee_updated_at`, falling back to the Mifflin-St Jeor formula. Optional arg keeps full back-compat for callers not yet upgraded. 5 new unit assertions in `tests/unit/tdee.test.ts` pin the contract. **Closes F-145 fully**: three drift sources (snapshot, `getEffectiveTDEE`, `resolveMaintenance`) now route to the same staleness-aware contract. |
...
| **F-148** | `AF7bS2DQrH_wZWxGosBJ3K8` | ✅ closed by-design 2026-05-10 | P1 weight chart | "Weight chart 3/6/9 month buttons don't change months shown". The 6M / 9M pills literally don't exist in `apps/mobile/components/charts/TimeRangeSelector.tsx:26` — current ladder is `1W / 1M / 3M / 1Y / All` (5 pills, matching MFP / Apple Health / Strava conventions). Closing as by-design per Grace's "keep going" direction (option b in the original triage): the labels themselves never claim 6M / 9M exist, so there is no misleading copy to fix. Reopen with option (a) — re-add 6M with a real `"6m"` bucket — only if testers report this again on a fresh build. |
| **F-149** | (recurrence Theme 3) | ✅ PR #176 + PR #179 | P1 numbers | **Mobile retune (PR #176)** + **web Settings retune (PR #179)** both backfill past-day daily_targets snapshots before writing new target values. Same helper (`backfillDailyTargetsFromProfile`), same posture: `upsert(..., { ignoreDuplicates: true })` protects existing snapshots; only gaps fill. Wired into `apps/mobile/components/recap/GoalPaceRetuneSheet.tsx` (mobile) + `src/app/components/Settings.tsx:handleActivityLevelConfirm` (web). Web backfill only fires when `recomputed` is non-null (i.e. when targets actually change), not on every preference change. Past days without logs now show the target that was effective on that day, not the new one — both platforms. **Proper Theme 3 fix (`goal_history` table) still parked** — schema decision worth its own PR. |
| **F-150** | `AKvgjnb` + `APGJJlg` (recurrence Theme 1) + new mentions | ✅ PR #175 | P1 portions | **Worst-case macro corruption closed**: `apps/mobile/lib/verifyRecipe.ts:317` previously did `unit === "g" && amount ? amount : 100` for the per-100g back-calc, so any non-gram unit (cup of rice, medium banana, etc.) divided macros by factor=1 and inflated per-100g by up to 58%. Now uses `measureToGrams` from the shared nutrition module. **Receiver-side fallback re-evaluated 2026-05-10**: the 5 deferred sites all turned out to be `onIngredientBarcodeScanned` (OFF-only) callbacks. `inferNaturalServingFromName` is gated to verified USDA SR/Foundation rows by contract (`isVerified: true`) and pattern-matches USDA descriptions like "Bananas, raw" — branded OFF products with their own `servingSize` field can't legitimately use it. The OFF `product.servingSizeG ?? 100` fallback is correct as-is. Closing F-150. |
| **F-151** | `APdpODtJDL8q2JhtGup6DK0` | ✅ PR #177 (telemetry) | P2 broken | "Notifications don't appear to be working". Token-registration paths in `apps/mobile/lib/expoPushToken.ts` previously logged failures to `console.warn` only — invisible in production. Now fires `expo_push_token_register_attempted` PostHog event on every attempt with `{ outcome, surface, error_message? }`. Outcomes: `ok` / `no_user` / `no_project_id` / `fetch_failed` / `db_failed` (registration); plus `no_permission` / `no_change` for the focus-refresh path. Both `registerExpoPushTokenForUser` (post-permission-grant) and `refreshExpoPushTokenIfChanged` (focus-effect) instrumented. **Next TF failure is correlated to a concrete code**; the actual fix follows once we know which step is breaking. Server-side push-send paths (`weeklyRecapPushRoute.ts` etc.) still uninstrumented — separate follow-up. |
| **F-152** | `AOOBv-1Ow` (recurrence Theme 7) | ✅ PR #175 + #176 + #181 | P2 polish | **Lock-emoji sweep**: PR #175 caught 2 of 3 locked-chip instances in `apps/mobile/app/(tabs)/planner.tsx`. PR #176 follow-up caught the remaining 3rd (line 2059). **Wider checkmark-emoji sweep (PR #181)**: 4 product-chrome `✓` glyph sites swapped for `Check` lucide icon — `apps/mobile/app/shopping.tsx:830` (all-checked badge), `apps/mobile/app/(tabs)/planner.tsx:2263` (in-tolerance macro pill), `apps/mobile/app/(tabs)/barcode.tsx:603` (Verified badge), `src/app/components/RecipeUpload.tsx:1377` (import-step done indicator). **Kept (intentional)**: `apps/mobile/app/shopping.tsx:405-410` shareable-text builder (`🛒 Shopping List`, `📌 ${cat}`, `☐ ${name}`) — user-shareable strings, not chrome. |
...
| **F-154** | (no specific ID, multiple mentions) | ✅ PR #182 (MVP) | P2 feature | "Share meal" action added to the meal-row context menu on both platforms. Mobile: `apps/mobile/components/today/TodayMealsSection.tsx` long-press alert (after Edit + Copy, before Delete) → `Share.share({ message })`. Web: `src/app/components/suppr/today-meals-section.tsx` kebab dropdown (after Copy, before Delete) → `navigator.share` with `clipboard.writeText` fallback + sonner toast. Shared text is built by `src/lib/share/buildMealShareText.ts` (7 unit tests, pinned privacy posture: meal nutrition only, NEVER user targets / day budget / identity — solves the "husband has different macros" complaint). New PostHog event `meal_share_invoked` with `surface` + `outcome` props (shared / dismissed / error). Format: `{recipe} · {portion} servings\n{kcal} kcal · {P}p {C}c {F}f\n\nvia Suppr`. |
| **F-155** | `AOI9xgY88Dx-uphiXI8IzEk` (recurrence of F-96) | ✅ verified wired | P2 broken→UX | Verified by code-walking: Edamam IS wired into the food-search path (`apps/mobile/components/food-search/FoodSearchPanel.tsx` handles `_source === "Edamam"` with macros + micros render at L687-705; `searchEdamam(t, { page })` in `verifyRecipe.ts:921` is called WITHOUT `mode` so the route's default `mode=foods` returns ALL hits including restaurant meals — only `mode=meals` (Discover) filters to restaurant-only). Edamam's `/parser` endpoint with `nutrition-type=logging` returns mixed generic + branded + restaurant hits. So restaurant items DO surface when Edamam has them. **Likely UX issue, not integration**: the source badge labels rows generically as "Edamam" (per FoodSearchPanel `source` union L146) — a row that is a restaurant menu item isn't visually distinguished from a branded ingredient. **Deferred UX improvement**: differentiate restaurant rows in the source badge (e.g. "Pret" branded chip when `brand` is set OR "Restaurant" when category contains "meal"). Verify against a fresh TF build first to confirm restaurant items are actually returned for tester's queries — Edamam may have sparse coverage for some chains. |
| **F-156** | (no specific ID, multiple mentions) | 🟠 PR #184 + #185 (PRs 1 + 2 of 4) | P2 feature | Spec at `docs/planning/F-156-custom-food-parity-spec.md`; 3 open questions resolved by Grace 2026-05-10. **PR-1 (#184) shipped:** basis toggle (per-serving / per-100g) + Fibre into the 2x3 macro grid + `convertMacrosBetweenBases` helper. **PR-2 (#185) shipped:** (M2) `BarcodeScannerModal.onAddAsCustomFood` + `today-barcode-dialog.onAddAsCustomFood` CTAs in the not-found state hand the scanned barcode to a CreateCustomFoodSheet/Dialog mounted in the host (`apps/mobile/app/(tabs)/index.tsx` + `src/app/components/NutritionTracker.tsx`) with `initialBarcode` pre-filled and the disclosure auto-opened — saving writes the row to `user_custom_foods` with the correct barcode so the next scan resolves successfully. (M3) Unlimited additional serving rows (Grace overrode the spec's cap of 3) — first row stays canonical (drives basis toggle + preview); extra rows are compact `[label] [grams] [×]` lines with both-or-neither validation per row. PR-3 (Cholesterol + Potassium read surface) + PR-4 (write fields + schema add) deferred. |
| **F-157** | (Grace 2026-05-10, screenshot in chat) | ✅ PR #176 | P1 weekly check-in | "Burn lower than expected but proposes increasing cals". Floor-binds reframe shipped in `src/lib/nutrition/weeklyCheckin.ts:buildWeeklyCheckinContent` lines 612-639: when burn dropped (`tdeeDeltaKcal < 0`) AND the floor clamped the suggestion upward (`floorAppliedKcal != null && suggestedTargetKcal > currentTargetKcal`), the `whyLine` now appends "Your goal pace would dip below the safety floor, so we're suggesting X kcal/day (a slower pace) instead." in the SAME prominent line — so the math + safety story are read together, not as separated facts. Two assertions in `tests/unit/weeklyCheckin.test.ts:585-620` pin both branches (floor-binds reframe path + no-reframe when floor doesn't bind). Verified on main 2026-05-11. |

**Recipe-related items (pre-launch wave, started 2026-05-10):**
- `AJHZNp8NHTiFNk9TjQfdYBk` — "Imported MFP carbs way higher than actual" (recipe import accuracy) — ⏳ deferred, awaiting tester repro (see `docs/decisions/2026-05-11-tf-feedback-reconciliation.md`)
- "Imported recipes lose source link" / "Esther Clark recipe — source not clickable" — ✅ PR #186. The two reports collapse to the same root: the image-import path (`POST /api/recipe-import/image`) was the only branch that didn't accept or return `sourceUrl` / `sourceName`, so saved rows had null attribution and the source-card link wasn't tappable. URL + caption branches were always correct. Fix: route now reads optional `sourceUrl` / `sourceName` form fields, runs through the shared `normaliseSource` helper, and returns sanitised values. Web (`RecipeUpload.tsx`) and mobile (`apps/mobile/app/import-shared.tsx`) both forward the URL the user already pasted in the URL-import field (silent reuse). 4 new integration tests pin the contract. "Esther Clark" closure: that recipe was image-imported, so it landed with no URL → fell through to the name-only static-text render branch (which is correct behaviour, but read as broken).
- "Some recipes pulling whole caption in as title" — ✅ closed previously (F-76 build 44, PR #125 + #163 follow-up).
- "Library inconsistency: some recipes have images, some don't" (the hero-images thread, paused pending strategy call) — ⏳ deferred, awaiting strategy call (see `docs/decisions/2026-05-11-tf-feedback-reconciliation.md`)
- "Defaults to recipes that don't exist" on some surface (recipe library state) — ⏳ deferred, awaiting triage on next TF build (see `docs/decisions/2026-05-11-tf-feedback-reconciliation.md`). Recipe-id FK cascades from PR #167 mean orphans degrade gracefully — no crash, just missing label.
...
6. **F-150** — 100g defaults: worst-case (verifyRecipe back-calc) closed in PR #175 via `measureToGrams`. The 5 receiver-side callbacks in barcode flows are correctly using `product.servingSizeG ?? 100` — no further work warranted (2026-05-10 re-evaluation).
...
|----|----|------|-----------|--------|
| **F-131** | `AMmlpVOqMnaKKdV2dobjjjg` | Today / burn-summary card | "Need to check this number is correct - would be helpful to click on here and see what it's made up of" — exact validation of Pattern #9. | ✅ shipped — `TodayActivityBonusCard.tsx:303-320` renders a small Info icon next to "{N} kcal burned so far" that fires `onShowBurnProvenance` → opens `WhereThisComesFromSheet` with the burn breakdown. Wired in `apps/mobile/app/(tabs)/index.tsx:5019` via `setProvenanceContext("burn")`. The weekly summary section below the daily row is derived data (7-day rolling aggregate of the same numbers); not adding a separate provenance sheet for derived aggregates is intentional — tapping the daily info icon explains the per-day math the weekly aggregate is built from. Closing 2026-05-10 on code-level argument; reopen if testers ask for weekly-specific provenance. |
| **F-132** | `AGNEODiMEK7F` + `ALP1WPzhIZqba0TBcWKw7gU` | Progress + Weight tracker | "Do we need weight graphs in 3 places? Do it once and do it well" + "Page is so slow to load". F-125 unified the chart components but not the surfaces — Progress page renders both a small weight widget AND a bigger weight section, plus the standalone `/weight-tracker` route. | ⏳ — UX consolidation pass deferred per `docs/decisions/2026-05-11-tf-feedback-reconciliation.md`. Not a launch blocker (charts render correctly; redundancy is polish). |
| **F-133** | `AFlB4oMfwQGIFx-w0DxOofE` | Weight & Trends | "Graph is broken" — the y-axis stretches to include the goal line (50 kg) when current is 54.6 kg, so the actual data line collapses to near-invisible at the top of the plot area. Looks like one floating dot with the rest empty. | ✅ already shipped (2026-05-08) — `computeYDomain` in `src/lib/progress/weightTrend.ts:163` only includes the goal in the domain when within `max(2, dataRange × 0.5)` of the data range. For the F-133 screenshot case (data 54.6, goal 50), the 4.6kg gap exceeds the 2kg threshold so goal is excluded; `WeightChart.tsx:195-202` then renders an edge chip indicating direction. Identical posture for `TrendLine.tsx` via `computeWeightChartDomain`. Test coverage: `apps/mobile/tests/unit/weightChartDomain.test.ts` G-3 case (10 tests passing) covers the exact 54.2-55.5 + goal 50 shape from the screenshot. Closure on code-level argument + test; no PR needed. |
| **F-134** | `AH2YKLI84Fc` + `AIwJjIxWtKm` + `ALYyTJ5iBRo` + `ADU-JU` | Barcode result sheet | **Same visual bug across 4 of 11 screenshots** — a rounded grey blob sits stacked behind the barcode result sheet. Looks like an orphan modal layer or a misplaced backdrop pill. P0 — visible on every barcode scan. | ✅ PR #183 — `BarcodeScannerModal.tsx` was already fixed 2026-05-08; the tab version `apps/mobile/app/(tabs)/barcode.tsx:521` had the same bug pattern. The reticle (rounded border at top:25%, height:30%, border-color = `Accent.primary + "80"` rendering as grey behind the result card) was gated only by `!product`. During the lookup pause `loading=true` but `product=null`, so the reticle stayed visible behind the "Looking up product…" spinner. Fix: extend the gate to `!product && !loading && !manualMode && !correctionMode` — matches the camera-active condition on line 516 plus a loading carve-out. |
| **F-135** | `ADU-JU-1zRIm2WQBeovKEjA` | Barcode result | "Log · 1 rice paper (~9" green button text wraps mid-token; "11.33 rice papers" chip uses absurd decimal precision. | ✅ already shipped 2026-05-08 — `BarcodeScannerModal.tsx:1295` strips the "(~Ng)" parenthetical from the Log button (the Amount field above already shows grams) + `numberOfLines=1 ellipsizeMode="tail"` safety net. Decimal precision collapsed via `tidyDecimalCount` (line 211) — leading-number `.0X` / `.9X` rounded to integer; meaningful halves (1.5 cups, 0.5 tbsp) preserved via the `0.1 < frac < 0.9` guard. Closure on code-level argument; no PR needed. |
| **F-136** | `AG5LqMGUpER2Gqi5N03_ytc` | Scan Barcode empty state | Red error icon for an informational state ("We don't have this product yet" isn't an error). 3 stacked CTAs ("Snap the label / Scan again / Enter manually") = decision fatigue. | ✅ already shipped 2026-05-08 — `BarcodeScannerModal.tsx:966-986` swapped the red error-icon for an informational icon and re-grouped the CTAs. Closure on code-level argument; no PR needed. |
| **F-137** | `ALYyTJ5iBRo3qzeXngFZclA` | Barcode result | "Need to be able to edit simply like 4 nuggets instead of edit the grams. Like it says 1 nugget I should be able to select that and say 4 of 1 nugget like mfp lose it etc". Portion chip needs a count multiplier. | ⏳ — UX design deferred per `docs/decisions/2026-05-11-tf-feedback-reconciliation.md`. Not a launch blocker (gram entry works; multiplier is paper-cut). |
...
| **F-139** | `ACyWRLx2M-_D9t2jdcNjmaU` | Today | "The goals hit banner looks cheap" — solid blue banner overlaps the calorie ring. Needs lighter visual weight + non-overlapping placement. | ✅ already shipped (2026-05-08) — `apps/mobile/app/(tabs)/index.tsx:5411` redesigned: previous full-width 88%-opacity `Accent.primary` block obscuring the calorie ring → white card with subtle `Accent.success + "60"` border + inline checkmark + soft shadow, repositioned to `top + Spacing.lg` (under the safe-area inset) so the calorie ring stays visible. Test pinned in `apps/mobile/tests/unit/goalsHitBannerRestyle.test.ts`. Note: Ionicons checkmark-circle is still used — separate F-152 scope. Closure on code-level argument; no PR needed. |
