...
| **F-156** | (no specific ID, multiple mentions) | 📋 spec ready | P2 feature | Product spec written: `docs/planning/F-156-custom-food-parity-spec.md`. Recommends two PRs of post-launch work: PR-1 (~1.5 days) per-serving-vs-per-100g basis toggle + Fibre into the macro grid; PR-2 (~2 days) scan-to-prefill from barcode-failure path + multi-serving array (cap 3). Schema is already ahead of the form — no migrations. Out of scope: photo of label, full micros grid, public/private (owned by F-138 P4). Three open questions for Grace logged in spec §"Open questions". |
