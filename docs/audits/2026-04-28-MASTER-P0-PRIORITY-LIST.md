...
27. 🟡 **"More" exists nowhere on web.** **IN FLIGHT — Group G IA verdict shipped via product-lead.** Decision: collapse You into 2 sub-tabs (Progress + Settings); Profile-as-row; kill standalone More tab. 5-batch plan in `docs/decisions/2026-04-28-group-g-ia-collapse.md`. Batch A shipped 2026-04-28 (pill collapse 3→2 mobile-web, commit a1b509e). Batch B shipped 2026-04-29 (mobile bundle extraction). Batch C shipped 2026-04-29 (web sidebar collapse + Settings header card). Batch D shipped 2026-04-30 — `/(tabs)/more` collapsed to a 17-line redirect to `/(tabs)/settings`; bundle now renders only on Settings; 4 Maestro flows repointed; verified on Grace's iPhone 17 Pro before merge. Batch E pending (delete `more.tsx` + drop `pathname.startsWith('/more')` after one release of grace period).
28. 🟡 **Settings + More 80% overlap on native.** **IN FLIGHT — same plan as #27.** Native overlap collapsed in Batch B 2026-04-29 (shared bundle). Web parity shipped in Batch C 2026-04-29 (sidebar collapse + header card). `/more` collapsed to a redirect in Batch D 2026-04-30. Batch E (final cleanup) remaining.
