...
# Part 2 — Named-comparable reads (Phase 2.3 of `/Users/graceturner/.claude/plans/i-m-really-struggling-to-goofy-rivest.md`)

**Generated:** 2026-05-17
**Frame:** premium-auditor two-axis verdict (Concept × Execution) per surface vs its named comparable per the matrix in `P0-proposal.md` § Named comparables.
**Captures:** 24 light-public baselines at `tests/e2e/visual-regression/web-baseline.spec.ts-snapshots/`, mirrored to `captures/P0/before/` via `scripts/copy-baselines-to-p0.sh`.
**Pre-prompt:** `defended-choices.md` (DC1–DC15) read before each surface.

This Part 2 supplements Part 1 (the 2026-05-14 code-reality audit) — it does not replace it. Part 1 catalogues the 23 specific proposal rows; Part 2 names where the surface as a whole sits against its best-in-class comparable. Items in Part 2 may be folded into the Part 1 proposal table as new rows.

## Cover sheet — Part 2

| Surface | Comparable | Concept verdict | Execution verdict | Top action |
|---|---|---|---|---|
| `/` landing | Linear | AT BAR | BELOW BAR | SUBTRACT hero eyebrow chip + sticky compact header NEW |
| `/pricing` | Whoop | AT BAR | CLOSE | Promote DC4 trust chips to card-level ribbon + fix Pro/Free CTA hierarchy inversion |
| `/login` + `/signin` + `/signup` | Notion | AT BAR | WEAK | Visually disambiguate the 3 routes (`/signin` `hideTabs` prop not honoured at render time) |
| `/onboarding` web entry | Cal AI | AT BAR (BETTER on voice — DC12) | WEAK on web entry; SOLID on tablet + mobile-app Welcome | Hold desktop layout at max-width-1280; port the tablet's floating demo cards to mobile-375 |
| `/today` (signed-in, modal-occluded) | MacroFactor | BETTER THAN BAR | WEAK | SUBTRACT persistent help-toast (bleeds across all 6 authed surfaces) + 5→2 top-right widget cluster |
| Sidebar shell (248px) | Linear | EQUAL | SOLID rail / WEAK bottom | Pin upgrade card (Free) or plan chip (Pro) in bottom slot; verify dev-tools chrome doesn't leak to prod |

---

## P2.1 — Landing (`/`) vs Linear

**Concept:** AT BAR vs Linear (single-product narrative scroll with hero ring spine + "core loop" tiles + comparator + paywall preview + FAQ + closer). Suppr's spine — kcal-centric ring + macro story — is genuinely bolder than Linear's screenshot-rotator hero.

**Execution:** BELOW BAR vs Linear (typography rhythm, vertical density, divider/section-card discipline, eyebrow-chip restraint, and the slate footer's contrast switch are all weaker than `linear.app/` today).

### BETTER-THAN-BAR carve-outs

1. **Hero ring + Today screenshot pair** [DC1, DC10 intact] — multi-ring product proof in the hero beats Linear's static screenshot-rotator. The full macro story inside the kcal centre is load-bearing differentiator; **defend the nested rings and the 3-state colour rule.**
2. **"Recipes you actually save, tracked the way food actually works."** — the four-row trust ladder (USDA-match / count-to-weight / per-ingredient parse / paste-link sources) is more specific than anything Linear has below its hero. [DC13 intact.]
3. **"Everything you need to eat well" Free £0 vs Pro £7.99 comparator** [DC4, DC15 adjacent] — VAT-inclusive `£7.99` digit, no asterisks, no "starting from". Linear-level honesty signal.
4. **"What's shipping, what's next" feed** — closer to Linear's `/changelog` voice; reads as operating signal, not marketing. Defend.
5. **"Questions, answered honestly."** — accordion FAQ headline matches Linear's directness. Defend phrasing.
6. **"Start cooking what the numbers say." pink-purple closer band** — gradient + product-rooted CTA copy stronger than Linear's flat dark slab. [DC12 intact.]

### SUBTRACT / TIGHTEN / REPLACE / NEW (ranked by first-impression impact)

1. **SUBTRACT — eyebrow chip above hero H1** (mobile-375 + desktop-1280). Competes with H1 and dilutes the first 200ms. Linear's hero has H1 first, eyebrow earned. **Drop the chip; let the H1 land.**
2. **TIGHTEN — hero H1 line-break and rhythm** (desktop-1280 + 1920). "Import any recipe. / Get real macros." stacks three short lines with too much leading. **Reduce line-height ~5%, set max-width so the second clause lands on one line at ≥1280.**
3. **TIGHTEN — "The core loop, done properly." 6-tile card** (mobile-375 mid-scroll + desktop-1280). Six tiny cards looks like a stock feature grid. **Reduce to 4 tiles, double per-tile vertical space, replace icon-circle stamp with product-rooted glyph treatment.**
4. **REPLACE — "One system. Phone for logging, web for planning." card** (mobile-375 lower-mid + desktop-1280 middle). Reads as marketing collage. **Replace with two stacked full-bleed product screenshots — phone Today on top, web Planner below — both at real density.**
5. **TIGHTEN — "What's shipping, what's next" feed cards** (mobile-375 ~70% scroll + desktop-1280 middle). Three stacked feed entries are visually identical, low-contrast text. **Add left-rail date stamp, bold entry title 18-20px, drop body to muted 14px.**
6. **TIGHTEN — divider discipline across sections** (all four captures). Hairlines flatten the rhythm. Linear uses generous vertical breathing room and tonal bg shifts. **Remove hairlines; alternate panel bg (near-white / faint warm tint) between sections.**
7. **NEW — sticky compact header on scroll** (absent in all four captures). Without it, by section 3 the user has no in-page CTA visible. **Add a 56px sticky compressed header with `Suppr` mark left, "Sign in" + "Get the app" right, appearing after 600px scroll.**

### Refuse-to-pass list

- **Hero rhythm + eyebrow chip stack** (item 1 + 2). First 200ms loses to Linear. Owner: `ui-product-designer`.
- **Core-loop 6-tile grid** (item 3). Reads as generic feature wall. Owner: `ui-product-designer`.
- **"One system" composite card** (item 4). Marketing collage, not product proof. Owner: `ui-product-designer` (needs fresh Today + Planner captures from `executor`).
- **Sticky compact header missing** (item 7). No in-viewport CTA after 600px scroll. Owner: `executor` (small build).

**DCs intact:** DC1, DC4, DC10, DC12, DC13, DC15. None at risk on landing.

**What AT BAR (Execution) looks like:** Hero opens H1-first with a tight two-line break, the multi-ring + Today screenshot pair reads as the product in 200ms, sections are separated by tonal panels not hairlines, the core loop is 4 larger tiles not 6, the phone-vs-web moment shows two real product screenshots stacked, a sticky 56px header keeps "Get the app" in-viewport from scroll 600px, and the closer band + Free/Pro comparator + paywall trust chips do their existing job uninterrupted.

---

## P2.2 — Pricing (`/pricing`) vs Whoop

**Concept:** AT BAR — Whoop runs a tier-card cold-open with above-the-fold trust ribbon and FAQ floor; Suppr mirrors the structural arc (hero → toggle → two cards → FAQ) and adds a free anchor Whoop doesn't have. Approach is sound.

**Execution:** CLOSE — the trust spine (DC4 chips, DC15 VAT, refund/cancel language) is present but rendered as small grey legal print; Whoop's same trust beats live as a top-of-card ribbon. The hierarchy mutes Suppr's strongest differentiators.

### BETTER-THAN-BAR carve-outs (defend)

1. **Free £0 anchor card beside paid** — Whoop has no free tier. Suppr's two-card layout makes Pro value legible against a real reference point. Defended.
2. **DC4 trust chips inline under the toggle** (`Cancel in Stripe Portal · 7-day refund, no email · Price never changes mid-trial`) — Whoop puts a single "Cancel anytime" near the CTA; Suppr triples the trust surface and counters MFP's mid-trial trickery at the cold-open decision moment. Defended.
3. **Annual savings chip "Save 38%" attached to the toggle itself** — Whoop's annual savings live inside the card body; Suppr surfaces them at the choice moment.
4. **Free tier feature list is full-length, not crippled** — Adaptive TDEE, recipe import from URL, barcode, export. The most credible "we are not predatory" signal on the page.
5. **DC15 VAT-inclusive £7.99 posture** with explanatory legal footer — Whoop (US-HQ) routinely fails this on UK surfaces. Compliance + trust win.
6. **FAQ leads with "Can I cancel anytime?"** — Whoop leads with product questions; Suppr leads with cancellation fear, then privacy. Cohort-aware question order — MFP-refugee priorities first.

### SUBTRACT / TIGHTEN / REPLACE / NEW (ranked by conversion impact)

1. **NEW — Promote DC4 trust chips from text row to a card-level ribbon.** Highest impact. Current chips render as a faint grey strip below the toggle, vertically squeezed against the cards. Whoop runs `30-day money-back guarantee` as a coloured chip directly above the price. Move the chip row inside or immediately above each Pro card with stronger contrast and a small check glyph — same chips, ten times the visual weight. (Borrow: chip styling and placement; do NOT borrow Whoop's single-claim minimalism — keep all three.)
2. **TIGHTEN — Pro card price hierarchy is buried.** `£7.99` reads at near-body-weight beside the dense feature column. Whoop's price sits in a dedicated row at ~36–40px with nothing else on its baseline. Increase typeface ramp, add ~16px breathing room above/below, demote `/month` to baseline-aligned small-caps unit. Same for `£59.99/year` on Annual toggle.
3. **REPLACE — "Continue for free" CTA on the Free card is the wrong colour/weight.** Renders as a quiet outlined button identical in weight to a tertiary link; Pro card's primary CTA appears clipped/missing in 1280 and 1920 captures — Free's CTA dominates the page, inverting conversion intent. Make Pro CTA the primary filled button; demote Free to text link or ghost button below the fold.
4. **SUBTRACT — Hero ribbon eats ~30% of desktop fold and ~45% of mobile fold.** Whoop's equivalent is one-line headline + thin sub. Cut sub-headline to single line ("Plan a week, log a meal, hit your macros"), drop the "10 secs · auto-saved · ditch the spreadsheet" microline (reads as a separate bolted-on component), recover ~120px of fold. Cards must enter the fold on desktop-1280 without scroll.
5. **TIGHTEN — Cancel/refund/export icon row at card bottom is redundant with DC4 chips.** Two trust surfaces in different visual languages weakens both. Either delete the icon row OR replace with three Stripe/App Store/RevenueCat trust-mark logos — actual third-party anchors, not self-claims.
6. **NEW — No social proof anywhere on the cold-open page.** Whoop runs reviewer quotes / athlete endorsements / star-rating block above the fold. For a £7.99/mo cold-open from TikTok referral, absence of third-party validation is the biggest single conversion gap after the trust-chip promotion. Add App Store rating chip (when available) OR one MFP-refugee testimonial OR a TestFlight user count — anything human. Voice filter: keep DC12 calm anti-shaming tone (a "finally a tracker that doesn't yell at me" quote fits; "lost 15lb in 2 weeks!" does not).
7. **NEW — Mobile FAQ is full-collapsed accordion with no preview text.** Whoop's mobile FAQ shows first ~40 chars of each answer to telegraph length and reduce tap-cost. Either preview first line OR auto-expand the top question ("Can I cancel anytime?") — the cancellation-anxiety question is the one MFP refugees land here to resolve, and it should be answered without a tap.

### Refuse-to-pass list

- **Pro card CTA hierarchy inversion** (item 3). Free CTA cannot remain visually dominant over Pro on desktop-1280 + 1920. Owner: `ui-product-designer` → `executor`.
- **DC4 trust chip promotion** (item 1). Chips exist; they are not being seen. Card-level ribbon required before paid-acquisition campaign. Owner: `ui-product-designer`.
- **Zero social proof on cold-open conversion surface** (item 6). Even a single quote or rating chip clears the bar. Blocking for TikTok/IG referral push. Owner: `ui-product-designer` for placement + voice guard.

**What AT BAR looks like:** Pro CTA is the loudest pixel on the page; DC4 chips render as a coloured ribbon attached to the card, not as grey footer text; `£7.99` sits in its own breathing room at display-size; one piece of third-party validation appears above the FAQ; hero collapses to one headline + one sub-line so both cards enter the desktop-1280 fold without scroll.

---

## P2.3 — Login / Signin / Signup vs Notion

**Concept:** AT BAR vs Notion. Suppr's three-route model (`/login`, `/signin`, `/signup`) with an in-card mode toggle is the correct call for an MFP-refugee cohort who needs to see "Sign in" and "Sign up" as separate, named actions rather than Notion's clever-but-opaque single-field-then-branch. Apple as primary OAuth is right.

**Execution:** WEAK. Three pixel-near-identical routes with no clear semantic distinction; `hideTabs` is not honoured in the `/signin` capture (toggle still visible); duplicate-headline problem from 2026-05-14 audit persists in card form ("Welcome back" + "Sign in to continue" is one sentence in two lines); Apple button uses `#000`/`#fff` literals (token drift); the disabled signup CTA renders as muddy purple-lavender that reads as a loading state, not as "unmet precondition".

**Pixel finding:** `/login` and `/signin` mobile/desktop captures are byte-near-identical — both show the Sign up / Sign in toggle with "Sign in" active. The `hideTabs` prop on `/signin/page.tsx` is set in code but not visible in the rendered pixel, suggesting the route is not wiring the prop through at render time OR the capture went through the redirect to `/login`. `/signup` differs only by: "Create your account" headline, "Free to start…" subhead, Sign up button active blue, Terms checkbox + disabled "Create account" CTA.

### BETTER-THAN-BAR carve-outs (defend)

1. **Apple OAuth above the fold, primary visual weight after the toggle.** Notion buries Google + Apple equally; Suppr's Apple-first ordering matches the mobile-app hierarchy (parity bet) and serves the iOS-only TestFlight cohort. Keep the black tile + glyph as heaviest non-primary element.
2. **Explicit Sign up / Sign in toggle is named, not inferred.** Notion's "type email then we'll figure out which" trick fails for MFP refugees with a strong "I have / don't have an account" mental model. Toggle is the right concept. Defended.
3. **Mode-dependent subtitle copy** ("Sign in to continue" vs "Free to start. Set your targets and plan your first week.") gives signup a real value-prop sentence Notion doesn't bother with. The signup subhead does real work.
4. **Explicit Terms + Privacy assent checkbox on signup, gating the CTA.** Notion's browsewrap is unenforceable under Nguyen v. Barnes & Noble — Suppr's positive-assent is legally stronger AND clearer. Code comment names the case law. Defended.
5. **Magic link as a calm secondary affordance** rather than the primary path. Notion forces magic-link-or-password-set-up-first; Suppr's "password primary, magic link opt-in" respects users with a password.
6. **Footer disclosure** ("If you just created an account, you may need to confirm your email before signing in.") — Notion has no equivalent; this kills a real support ticket.

### SUBTRACT / TIGHTEN / REPLACE / NEW

1. **REPLACE — `/login` vs `/signin` vs `/signup` visual disambiguation.** Pixel evidence: `/login` and `/signin` are byte-identical. `/signin` should hide the toggle entirely (honour `hideTabs`) and add the existing "New to Suppr? Create your account" cross-link as the only mode-switch affordance — that pattern is coded but not visible in the capture. `/signup` should hide the toggle and add a symmetric "Already have an account? Sign in" cross-link. `/login` keeps the toggle as the "I haven't decided yet" landing.
2. **SUBTRACT — collapse subtitle into headline on signin.** "Welcome back" + "Sign in to continue." is the same sentence twice. Keep "Welcome back." Drop the subtitle on signin mode only.
3. **TIGHTEN — Apple button literals.** `#000` / `#fff` / `1px solid #000` are token bypasses (Apple HIG requires `#000` so the value is right — but it must come through a named token so dark-mode and future variants can ship). Add `var(--apple-bg)` / `var(--apple-fg)` tokens.
4. **TIGHTEN — disabled "Create account" CTA reads as loading.** Washed purple/lavender with white text reads as a spinner state, not "you need to check Terms". Use `opacity-50` on actual primary colour (current Tailwind class is there but `boxShadow` override is fighting it). Add text affordance ("Agree to continue") below disabled button OR move Terms checkbox closer so cause-and-effect is visceral.
5. **TIGHTEN — input fields use 2px borders + low-contrast `var(--input-background)` fill.** At 375 fields read as plain grey rectangles. Notion's borderless underline-style inputs look more premium. Drop to 1px borders, lift fill to pure white in light mode, rely on `var(--ring)` focus-ring shadow for affordance.
6. **NEW — "Forgot password?" sits below CTA as 14px primary-blue link with no separator.** Notion uses a calm caption-style link. Demote to `var(--muted-foreground)` with primary-blue on hover. Tighten spacing.
7. **NEW — Brand mark above card is 48×48 with heavy shadow.** Notion's brand mark is 32×32, no shadow, sits inside the card top. Either: (a) drop shadow + shrink to 32×32, or (b) absorb mark inside card top-left at 24×24 with headline beside it.

### Refuse-to-pass list

| Surface | Comparable | Blocker | Owner |
|---|---|---|---|
| `/signin` route | Notion single-purpose auth route | `hideTabs` prop set in code but captured pixel still renders toggle. Route non-distinct from `/login`. | `executor` (verify + fix wire-up); `visual-qa` (re-capture) |
| `/signup` route | Notion named-intent route | No cross-link back to `/signin` for users who realise they have an account. Asymmetric. | `executor` |
| Signup CTA disabled state | Stripe Checkout disabled CTA | Renders as loading-purple, not "precondition unmet". Visual ambiguity at most important moment. | `visual-qa` |
| Apple button token | Design system | `#000`/`#fff` literals in `app/login/ui.tsx:293-295`. Bypasses tokenisation. | `design-system-enforcer` |

**What AT BAR looks like:** Three routes each immediately recognisable from above-the-fold pixels alone. `/login` = "haven't decided" with toggle. `/signin` = no toggle, "Welcome back" only, "New to Suppr?" cross-link in footer. `/signup` = no toggle, "Create your account" + value-prop sub, Terms checkbox inline-above CTA, "Already have an account?" cross-link. Apple button through tokens. Disabled CTA reads "blocked" not "loading". Brand mark small inside card or removed.

---

## P2.4 — Onboarding (`/onboarding`) vs Cal AI

**Concept:** AT BAR vs Cal AI on web entry; **BETTER THAN BAR** on voice/positioning (DC12 calm voice vs Cal AI's growth-shouty "Lose weight with AI" hook).
**Execution:** WEAK on web entry (mobile-375 + desktop-1280/1920); SOLID on tablet-768 and on mobile-app Welcome.

**Capture-fix flag CF-1 — CONFIRMED.** `mobile-onb-03-goal.png` is a Maestro splash ("Downloading 100%…" + grey wireframe S). `mobile-onb-07-weight.png` and `mobile-onb-12-reveal.png` are both the **Welcome screen**, not weight/reveal. All 13 `mobile-onb-*` captures are wedged on Welcome or splash. Post-Welcome mobile steps are **uncaptured** — verdict on goal/sex/age/height/weight/activity/diet/strategy/reveal/permissions cannot be issued from pixels per `feedback_premium_audit_requires_pixels.md`. **Post-Welcome step verdicts deferred to next sweep once the Maestro flow is fixed.**

### BETTER-THAN-BAR carve-outs (defend)

1. **Calm-tech voice (DC12).** "Eat well, without overthinking it" on mobile; "Join the Suppr Club" on web — both anti-shaming. Cal AI opens with weight-loss numerals and growth-shouty energy. This is the **load-bearing reason the MFP exodus would switch**; do not push toward Cal AI's voice.
2. **Mobile Welcome example card with USDA trust stamp.** Sheet-pan chicken from instagram.com + USDA-backed nutrition chip is a **product demo in the cold-open** — Cal AI shows a generic food photo with a calorie estimate. Suppr shows the actual viral-loop bet (DC13 social-video import) in 2 seconds. Above the bar.
3. **Cross-platform divergence is intentional and project-memory-logged.** Web = anchor framing for cold marketing traffic; mobile = product demo for an audience past the install gate. Different jobs, different copy. Defended.
4. **Three falsifiable trust receipts under the CTA on web** ("Adaptive TDEE that learns from you · One-tap import from any recipe site · Calm design, private by default"). Cal AI uses unverifiable testimonials. Suppr's checks-with-green-tick triplet is **specifics, not vibes** — same trust-chip pattern as DC4 on paywall, reinforced earlier in funnel.
5. **`robots: index:false`** on the route — disciplined. `/pricing` + `/` are the indexable surfaces. Cal AI indexes its onboarding stub, polluting search.
6. **Adaptive TDEE in Free tier (DC11) named at entry.** "Adaptive TDEE that learns from you" is **MacroFactor's $11.99/mo wedge surfaced free, on the cold screen**. Cal AI has no equivalent receipt.
7. **Sex-step inclusive helper (DC7) and Weight-skip path (DC6)** exist downstream — both above-bar vs Cal AI's forced sex/weight gates. Defended; do not collapse this sweep.

### SUBTRACT / TIGHTEN / REPLACE / NEW (web entry only — pixel-grounded)

1. **NEW — Visual demo above the fold on web (mobile-375 + desktop-1280/1920).** Mobile-375 shows headline + body + CTA + 3 receipts and **zero product surface** until you scroll. Tablet-768 already shows the right move: floating "What we match / 380 left / Sheet-pan chicken / Weekly insights" cards in the right column. **Port that card stack to mobile-375 (stacked below CTA at ~60% scale) and to desktop-1280/1920** (currently squeezed into 35%-width column at 1280, floating in negative space at 1920). Cal AI shows a calorie ring demo in cold-open; Suppr should show multi-ring spine (DC1) the same way. **Allowed borrow per DC1: Cal AI 400ms count-up animation on the demo ring.**
2. **SUBTRACT — Wordmark + S-glyph stacked at top of mobile-375.** The 32×32 S-glyph + "Suppr" wordmark above "Join the Suppr Club" creates a 3-element brand block where 1 would do. Cal AI uses a single inline glyph. Drop the wordmark; keep the glyph as a 28pt rounded square. Saves ~64pt vertical, lifts headline above the fold on iPhone SE.
3. **TIGHTEN — "Join thousands tracking smarter" micro-line above CTA is dead weight.** Vague social proof — exactly the "trusted by N users" pattern DC4 forbids on paywall. Either name a number ("3,400 imports this week") with a real source OR subtract entirely. Currently a 12pt grey orphan between body and CTA.
4. **TIGHTEN — "Join the club — free" CTA label is two-clause and ambiguous.** Reads as membership pitch, not product start. Mobile-app Welcome uses "Get started" — better. **Replace with "Get started — free"** on web for parity, or "Start free, no card" to mirror DC4 paywall posture.
5. **REPLACE — Desktop-1280 and -1920 layouts are broken.** At 1280 left column collapses to ~40% width, demo cards squeeze into tight right rail; at 1920 cards float in 800pt of empty gradient. **Hold layout at max-width-1280, demo cards in a fixed 480pt right column, centre the whole thing.** Currently reads as "we forgot desktop." Comparable here = Linear / Stripe / Notion onboarding cold-opens — all hold a max-width and centre.
6. **NEW — Persistent progress indicator from step 1.** Cal AI shows a top-of-screen segmented progress bar from screen 1, so users know commitment depth (~15 segments). Web entry has none; mobile-app Welcome has none. **Add a 4-segment progress chip ("Step 1 of ~12") at top of every step after Welcome** — Welcome itself stays chrome-free. Single biggest known onboarding-funnel lever in the category.
7. **SUBTRACT — "I'm already a member" link beneath primary CTA is button-styled.** Competes with primary CTA on mobile-375. Cal AI's sign-in link is a quiet text link top-right. **Demote to a 14pt grey text link** ("Already a member? Sign in") underneath the receipts row, not under the CTA.

### Refuse-to-pass list (web entry only — mobile-app steps deferred until CF-1 fixed)

| # | Surface | Comparable | Blocker | Owner |
|---|---|---|---|---|
| **R1** | Desktop-1280 + desktop-1920 layout | Linear cold-open | No max-width; cards float in dead space at 1920, squeeze at 1280. Reads broken on the two largest viewports. | `ui-product-designer` → `executor` |
| **R2** | Mobile-375 has no product demo above the fold | Cal AI ring teaser | Tablet-768 has the floating card stack; mobile-375 is text-only. Port the stack down. DC1 allowed borrow: 400ms count-up. | `ui-product-designer` |
| **R3** | "I'm already a member" button competes with primary CTA on mobile-375 | Cal AI / Linear quiet sign-in link | Demote to 14pt text link below receipts row. | `visual-qa` (cleanup) |
| **R4** | "Join thousands tracking smarter" vague social-proof orphan | DC4 specificity rule | Name a falsifiable number or subtract entirely. Currently violates DC4 a step earlier in funnel. | `ui-product-designer` |
| **R5** | No persistent progress indicator on post-Welcome web steps | Cal AI segmented progress | Add 4-segment "Step X of ~12" chip from step 2 onward. Single biggest funnel-completion lever in category. | `ui-product-designer` → `executor` |
| **R6** | CF-1 — Maestro flow wedged on Welcome | n/a — infra gap | All 13 `mobile-onb-*` captures are splash or Welcome. Post-Welcome verdict deferred. | `executor` (Maestro), then `premium-auditor` re-run |

**What AT BAR or BETTER looks like:** AT BAR web entry = max-width-centred layout on desktop with the right-column demo cards anchored at 1280; mobile-375 shows a stacked product demo below the CTA; the "I'm already a member" link is a quiet text link; the vague social-proof line is gone or specific; progress chip appears from step 2 onward; CTA reads "Get started — free." BETTER THAN BAR = on top of all that, the demo card stack on mobile-375 shows the live calorie ring (DC1) with the 400ms count-up animation borrowed from Cal AI — turning the entry from text-with-receipts into the product itself, while keeping the calm voice Cal AI cannot copy.

---

## P2.5 — Today (signed-in) vs MacroFactor & Sidebar shell vs Linear

**Capture caveat:** Today desktop capture has the "61 days of meal logging" milestone modal blocking the centre. Auditor read PAST the modal. The modal is itself a refuse-to-pass item; treat it as overlay-only and skip when judging the underlying surface.

### A. Today first-render vs MacroFactor

**Concept:** BETTER THAN BAR. Multi-ring spine + REMAINING/CONSUMED toggle + persistent LAST 7 DAYS rail is a richer ambient read than MacroFactor's calorie-arc-only hero with macros tucked below.

**Execution:** WEAK. Hero is suffocated by chrome (milestone modal, persistent help-toast, 4 top-right widgets, "Start your streak" pill) and the macro tiles render at zero-state without any "log to begin" affordance.

#### BETTER-THAN-BAR carve-outs (defend)

1. **Multi-ring calorie + macros spine** (DC1) — 1,224 centre + 4 macro arcs encode the day's full story. MacroFactor isolates calories from macros across two cards; we don't. Keep. Allowed borrow: 200ms ease-out fill on log (Apple Watch Move ring) — currently absent.
2. **REMAINING / CONSUMED pill toggle** — one tap, both mental models for the same ring. MacroFactor offers no equivalent. Keep verbatim. Allowed borrow: 200ms count-up tween when toggling, tabular-nums so digits don't bounce width on swap.
3. **LAST 7 DAYS right-rail with intra-day bars + total** — persistent context the user doesn't have to navigate. MacroFactor buries this behind a Trends tap. Keep. Allowed borrow: per-row tap → open that date's Today.
4. **Calorie ring 3-state colour rule** (DC10) — gradient/green/red restraint vs MacroFactor's uniform calorie arc. Keep. No borrow needed.
5. **Calm voice spine** (DC12) — Today doesn't shame at 0/41g. MacroFactor's "tighten up" copy at low-protein moments is the wrong voice for the MFP-refugee cohort. Keep.

#### Items ranked by daily-use retention impact

1. **SUBTRACT the persistent "You're all set! Keep logging on the Tracker..." help-toast.** Confirmed across 6 authed captures (Today, Progress, Library, Settings, Discover, Recipes). One-time post-onboarding nudge stuck on. Dismissed-state should persist to localStorage and never re-appear post-1st-log. **MacroFactor never reserves hero pixels for help microcopy** — they ship it as a single-shot post-onboarding sheet. *Retention impact: HIGH — every day, eyes lose 80px to a toast they can't action.*
2. **TIGHTEN top-right cluster from 5 widgets → 2.** Today renders: "Start your streak" pill, theme toggle, layout grid toggle, "G" chip, bell. Five tap targets for an authed hero whose job is "log fast." MacroFactor's Today has one icon (bell). Keep the bell. Push theme + layout into Settings (set-once preferences). Move streak pill into date header (DC8). *Retention impact: HIGH.*
3. **REPLACE the FAT / FIBER zero-state tiles' "0 / 41g · 41g remaining" double-statement.** Same number twice in different units of meaning. MacroFactor shows a single empty-state tile with 1-tap "Log first meal" affordance per macro. Either show arc-empty visual with target as subdued caption, OR collapse to "tap to log" CTA tile until first log of day. Current reads like debug print. *Retention impact: HIGH — first-log moment of every new day.*
4. **NEW: tap-to-Log on each macro tile.** Currently tiles are read-only — LOG is one global button. MacroFactor's daily-use power-move is **tap-the-macro → quick-add scoped to that macro** (tap Protein → recent protein-dense foods). Borrow verbatim. *Retention impact: VERY HIGH — quick-log latency drops 4 taps → 2.*
5. **TIGHTEN the milestone modal cadence.** A 61-day modal blocking the entire hero is a celebratory interrupt — fine quarterly, wrong daily. MacroFactor uses a non-blocking confetti chip in the header. Convert blocking modal → inline header milestone chip with "View summary" action. *Retention impact: MEDIUM — but daily-driver-hostile.*
6. **REPLACE "Today" centered page title with the date.** Header reads "Today" inside a week strip already saying "Today". Double-stamp. MacroFactor uses actual date ("Sun, May 17") as H1 — denser, removes redundancy. Pair with calm streak pip per DC8. *Retention impact: MEDIUM.*
7. **NEW: empty macro tiles need a 1-line "what to eat" hint when remaining > 80%.** This is the DC2 north-star moment surfaced **on the macro tile itself** when no log exists — "Hit 41g fat: try Salmon Sushi Bowl (3% fit)." MacroFactor has no equivalent. *Retention impact: HIGH — turns zero-state into the moat.*

#### Refuse-to-pass (Today)

| Surface | Comparable | Blocker | Owner |
|---|---|---|---|
| Persistent green help-toast (all 6 authed captures) | MacroFactor (no help-toast on hero) | Toast renders every authed surface, every visit. Dismiss never persists. **P0**. | `executor` (localStorage gate) |
| FAT/FIBER zero-state tiles | MacroFactor empty-tile pattern | "0 / 41g · 41g remaining" double-statement; no log affordance. **P0**. | `ui-product-designer` |
| Milestone modal blocks hero | MacroFactor non-blocking confetti chip | 61-day modal occludes ring + macro tiles. **P0** — daily-driver-hostile. | `ui-product-designer` |
| Top-right 5-widget cluster | MacroFactor (1 bell icon) | Theme + layout + streak + "G" + bell competing with LOG. **P0**. | `ui-product-designer` |

**What AT BAR looks like:** opening Today on a daily-driver returning user shows date + calm streak pip, the calorie ring + 4 macro arcs as the eye's first landing, each macro tile tappable for a scoped quick-add, no persistent help-toast, milestone moments surface as a header chip, and LAST 7 DAYS rail stays as the only persistent secondary read.

---

### B. Sidebar shell (248px) vs Linear

**Concept:** EQUAL — 4-primary + sub-tab disclosure + Cmd+B collapse mirrors Linear's left-rail spine precisely. Same muscle-memory, same width, same toggle shortcut.

**Execution:** SOLID for the rail itself; WEAK for the bottom-zone use (empty, plus a third-party dev-tools badge sits in the bottom-left chrome and reads as part of Suppr in some captures).

#### BETTER-THAN-BAR carve-outs (defend)

1. **Sub-tab indentation with vertical guide-line** — Recipes → Library/Discover, More → Progress/Settings render with `border-l border-border/60 pl-3` guide. Linear uses the same pattern. Equal. Keep.
2. **Sub-tab badge slot** (`libraryRecipeCount`, `shoppingUncheckedCount`) with "99+" overflow rule already wired. Linear has badges; ours render right-aligned and pill-shaped. Equal. Keep.
3. **Cmd/Ctrl+B parity** + `localStorage` persistence + input/contenteditable skip so search-field `⌘B` doesn't collide. Correct Linear/VS Code/Notion behaviour. Keep.
4. **Single "S" brand glyph + Suppr wordmark at 32×32** — cleaner than Linear because flat (no workspace switcher overhead that 95% of personal-account users never need). Keep.

#### Items

1. **NEW: free-tier upgrade card pinned bottom of rail.** Confirmed still missing across Library / Settings / Progress / Discover / Today captures — every sidebar bottom is empty whitespace. Linear pins a workspace-context card; for Suppr the bottom-slot bet is the **upgrade card for Free users** (mirror prototype). Pro users should see a 1-line "Pro · gracemturner" chip in the same slot so the bottom-zone is never empty. *Matches 2026-04-27 design-system-drift backlog — gap stands.*
2. **SUBTRACT the dev-tools "N" / "2 Issues" badge from production captures.** Next.js dev indicator sits in sidebar bottom-left in 4/6 captures, even renders as red "2 Issues" chip in Library shot. Dev-only chrome should not be visible at prod build. Verify `next.config` `devIndicators` posture and confirm prod builds don't render it.
3. **TIGHTEN sub-tab vertical spacing on More group.** Progress + Settings render as 2-row stack with `space-y-0.5` and `py-1.5`. Linear's equivalent uses `py-1`. Current is correct but reads slightly loose against a 2-row group; either accept (parity) or polish (-1px).
4. **NEW: keyboard navigation through primary items.** Linear supports `J`/`K` or arrow keys to move focus down/up the rail without leaving keyboard mode. Our sidebar relies on tab order only. Borrow Linear pattern for power-users — interaction-not-structural, calm voice, desktop-only.
5. **REPLACE the brand "S" badge's solid blue fill hover state** — currently no hover affordance on the brand row, meaning the row reads as static when it's actually the route-home click target on most Linear-style sidebars. Either add a subtle hover ring OR explicitly disable click on the brand row (consistency).

#### Refuse-to-pass (Sidebar)

| Surface | Comparable | Blocker | Owner |
|---|---|---|---|
| Bottom-slot of sidebar | Linear workspace card pin | Empty for Free users (no upgrade card) and empty for Pro users (no plan chip). Phase 1 backlog item still un-actioned. **P1**. | `ui-product-designer` |
| Dev-tools "N"/"2 Issues" badge | Linear (no dev chrome in prod) | If this renders in production builds it reads as Suppr chrome. Verify + gate. **P0 if prod, P3 if dev-only**. | `executor` (verify `devIndicators`) |

**What AT BAR looks like:** sidebar opens at 248px with Today/Recipes/Plan/More primary rows, active row carries pale-primary background, expanded primary discloses sub-tabs with vertical guide and right-aligned badges, the bottom slot pins an upgrade card (Free) or plan chip (Pro), `⌘B` collapses to 64px icon rail in 200ms, and no dev-tool chrome leaks into the rail in production.

---

## Part 2 — bucket summary

Five P0 surfaces audited against named comparables. Strongest finding: **Today has BETTER THAN BAR concept vs MacroFactor but WEAK execution suffocated by chrome.** The single highest-leverage retention fix is removing the persistent help-toast — it bleeds across all 6 captured authed surfaces (confirmed by pixel reads), so a one-line localStorage fix is a multi-surface premium-bar win.

**Cross-surface P0s (one fix → multi-surface lift):**
1. **Help-toast persistence bug** — affects every authed surface. Owner: `executor`.
2. **Dev-tools chrome leak verification** — Sidebar bottom-left. Owner: `executor` (`next.config` `devIndicators` check).
3. **Sidebar bottom-slot empty** — affects every authed surface. Owner: `ui-product-designer`.

**Per-surface P0s (ranked by impression weight):**
- **Landing**: hero rhythm + eyebrow chip stack; 6→4 core-loop tiles; sticky compact header.
- **Pricing**: Pro/Free CTA hierarchy inversion; DC4 chip promotion to ribbon; zero social proof.
- **Login/Signup/Signin**: 3-route visual disambiguation (`hideTabs` not honoured at render); Apple button token.
- **Onboarding** (web entry): desktop layout broken at 1280/1920 (no max-width); mobile-375 has no product demo; CF-1 Maestro flow wedged on Welcome blocks post-Welcome verdict.
- **Today**: persistent help-toast, 5→2 widget cluster, macro tile zero-state replacement, tap-to-Log scope.

All five surfaces have at least one BETTER-THAN-BAR carve-out worth defending — none are below-bar on Concept. This is the right shape for a launch surface set: design intent is sound, execution polish is the gap.

**Next gates:** G2 (Grace reads + flags wrong comparables / missed DCs); G3 (Grace red-lines per-row); G3.5 (HTML prototypes built for approved items before any code lands).
