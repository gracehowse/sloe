 ...
 *
 * As of 2026-04-20 the canonical onboarding lives at `/onboarding/v2`
 * and the parent server page (`./page.tsx`) issues a 307 redirect there
 * unconditionally. This form is only rendered when `?legacy=1` is
 * present on the URL — kept as a one-week fallback against any v2
 * regression. Once the validation window closes (≥ 2026-04-27) this
 * file and the `?legacy=1` hatch should be deleted together.
 *
 * Notes for the eventual deletion PR:
 *  - `app/onboarding/page.tsx` becomes `redirect("/onboarding/v2")` only
 *  - `subscribeToFlags` / `isOnboardingV2Enabled` may be removable from
 *    `src/lib/analytics/track.ts` if no other surface depends on them
 */
