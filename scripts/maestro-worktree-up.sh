...
# 2a-bis. apps/mobile/.env.local — Expo reads `EXPO_PUBLIC_*` from here
#         at bundle time. We need the FLAG_FORCE overrides so the
#         validation flow can deterministically render the flag-on
#         variant (PostHog's local-eval races against first render in
#         dev — the override sidesteps that). Don't symlink: main's
#         file only has SENTRY_AUTH_TOKEN (build-time), and we don't
#         want FLAG_FORCE leaking into the main-clone Metro.
#
#         Force-flags list intentionally lives in this script so the
#         validation flow + its overrides are co-located. To force
#         another flag, add another `cat >>` line below.
MOBILE_ENV_FILE="$MOBILE_DIR/.env.local"
if [[ ! -f "$MOBILE_ENV_FILE" ]] || ! grep -q "EXPO_PUBLIC_FLAG_FORCE_TODAY_LOG_USUAL_ROW_V2" "$MOBILE_ENV_FILE" 2>/dev/null; then
  echo "[worktree-up] writing apps/mobile/.env.local with FLAG_FORCE overrides for validation flows"
  cat > "$MOBILE_ENV_FILE" <<'ENV'
# Auto-generated by scripts/maestro-worktree-up.sh.
# Forces specific PostHog feature flags ON in this worktree's dev
# bundle so Maestro validation flows can exercise flag-gated UIs
# deterministically. Production builds drop this whole branch via the
# `__DEV__` guard in `lib/analytics.ts#isFeatureEnabled`.
EXPO_PUBLIC_FLAG_FORCE_TODAY_LOG_USUAL_ROW_V2=true
ENV
fi
