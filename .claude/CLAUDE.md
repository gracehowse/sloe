...
- **`Platform foundations`** — non-time-bound technical infra. Projects: Schema refactor, Operations, Design system cleanup, Post-iOS platform, **Deferred work backlog** (added 2026-07-21 — see below), **CI & test infra** (added 2026-07-21 — see below), **AI features** (dual), **Synthetic persona testing**, Architecture enablers (ex-Premium-P5).
...
**`CI & test infra` (Platform foundations project, added 2026-07-21):** the home for bugs IN the test/CI machinery itself — hanging/flaky tests, false-positive gates, env-parity divergence between local `npm run ci` and GitHub Actions, sweep/screenshot-harness defects. Explicitly distinct from **`Operations`**, whose own documented scope excludes "product/tech work of any kind" (it's runway snapshots, vendor renewals, compliance sweeps) — three tickets (ENG-1090, ENG-1559, ENG-1569) had already drifted into Operations before this project existed; leave them, but route new CI-infra tickets here instead. Also distinct from a product bug that merely surfaced via a failing test — that bug's home is wherever its product surface lives, not here. First occupants: ENG-1337, ENG-1501, ENG-1583.
